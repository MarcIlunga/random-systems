...
-/
import RandomSystems.MaxWinProb
import RandomSystems.PreWinFactorization
...
noncomputable def winProb (W : Dist (PFunDDS.Winner X Y)) (G : PFunPDS X (Y × Bool)) : ℝ :=
...
noncomputable def notWonProb (W : Dist (PFunDDS.Winner X Y)) (G : PFunPDS X (Y × Bool)) : ℝ :=
...
theorem winProb_add_notWonProb (W : Dist (PFunDDS.Winner X Y)) (G : PFunPDS X (Y × Bool)) :
    winProb W G + notWonProb W G = W.weight * G.weight :=
  GamePerf.winProb_add_compl winsDDS W G
...
noncomputable def maxWinProb (G : PFunPDS X (Y × Bool)) : ℝ :=
