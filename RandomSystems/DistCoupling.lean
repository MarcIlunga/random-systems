...
/-!
# Gluing two laws along a common projection

Two finite-support laws `left right : Dist A` that push forward to the **same**
law along a map `r : A → D` can always be joined: there is a law
`joint : Dist (A × A)` carried by the fibre relation `r x = r y` whose two
marginals are `left` and `right`.  The witness is the fibrewise product
...
gluing lemma.  The only delicate point is a fibre of measure zero, where the
definition divides by `0`; in `NNReal` that returns `0`, which is the right
answer, because such a fibre carries no mass on either side
...
The section also records the three general `Dist` facts the construction needs
...
`apply_le_fTransform_apply` (a point never outweighs its fibre).
-/
...
instance. -/
theorem mass_eq_sum_support (X : Dist A) (P : A → Prop) [DecidablePred P] :
...
theorem fTransform_apply_eq_sum_support (f : A → B) (X : Dist A) (b : B)
...
/-- **A point never outweighs its fibre** (for a non-negative law — over the
signed carrier a fibre can cancel below one of its points).  This is what
makes the division in the coupling harmless: the divisor vanishes only on
fibres that are null on both sides. -/
theorem apply_le_fTransform_apply (f : A → B) {X : Dist A} (hX : X.NonNeg)
...
/-! ## The fibrewise product coupling -/

section Coupling

variable [DecidableEq D]

/-- **The fibrewise product coupling.**  `left` and `right` are made independent
conditionally on their common projection along `r`; the normalizing divisor is
the mass of the fibre. -/
noncomputable def fibreCoupling (r : A → D) (left right : Dist A) : Dist (A × A) :=
...
@[simp] theorem fibreCoupling_apply (r : A → D) (left right : Dist A) (pair : A × A) :
    fibreCoupling r left right pair =
      if r pair.1 = r pair.2 then left pair.1 * right pair.2 / fTransform r left (r pair.1)
      else 0 :=
  Finsupp.onFinset_apply

/-- **The coupling is carried by the fibre relation.** -/
theorem eq_of_mem_support_fibreCoupling {r : A → D} {left right : Dist A} {pair : A × A}
    (member : pair ∈ (fibreCoupling r left right).support) : r pair.1 = r pair.2 := by
  by_contra different
  exact Finsupp.mem_support_iff.1 member (by simp [different])

/-- The fibrewise product of non-negative laws is non-negative — the
property the signed gluing witness below cannot offer. -/
theorem fibreCoupling_nonNeg (r : A → D) {left right : Dist A}
    (hleft : left.NonNeg) (hright : right.NonNeg) :
    (fibreCoupling r left right).NonNeg := by
  intro pair
  rw [fibreCoupling_apply]
...
theorem mass_fibreCoupling (r : A → D) (left right : Dist A) (P : A × A → Prop)
    [DecidablePred P] :
    (fibreCoupling r left right).mass P =
...
theorem fTransform_fst_fibreCoupling (r : A → D) {left : Dist A}
    (hleft : left.NonNeg) (right : Dist A)
    (project : fTransform r left = fTransform r right) :
    fTransform (Prod.fst : A × A → A) (fibreCoupling r left right) = left := by
  classical
  ext point
  rw [fTransform_apply_eq_mass, mass_fibreCoupling, Finset.sum_product]
  ...
  · rw [if_pos member]
    ...
    have fibrePos : fTransform r left (r point) ≠ 0 := fun zero =>
    ...
    rw [Finset.sum_congr rfl (fun y _ => factor y), ← Finset.mul_sum,
      ...
      div_mul_cancel₀ _ fibrePos]
  · rw [if_neg member, Finsupp.notMem_support_iff.1 member]

/-- **The second marginal of the coupling is `right`.** -/
theorem fTransform_snd_fibreCoupling (r : A → D) (left : Dist A)
    {right : Dist A} (hright : right.NonNeg)
    (project : fTransform r left = fTransform r right) :
    fTransform (Prod.snd : A × A → A) (fibreCoupling r left right) = right := by
  classical
  ext point
  rw [fTransform_apply_eq_mass, mass_fibreCoupling, Finset.sum_product]
  have collapse : ∀ x ∈ left.support,
  ...
  · simp only [if_pos member]
    ...
    rw [Finset.sum_congr rfl (fun x _ => factor x), ← Finset.mul_sum,
      ← mass_eq_sum_support, ← fTransform_apply_eq_mass, div_mul_cancel₀ _ fibrePos]
  · rw [Finsupp.notMem_support_iff.1 member]
    simp [member]
...
fibrewise product coupling.  Use this form whenever the joint must stay an
...
theorem exists_nonneg_coupling_of_fTransform_eq (r : A → D)
    {left right : Dist A} (hleft : left.NonNeg) (hright : right.NonNeg)
    (project : fTransform r left = fTransform r right) :
    ∃ joint : Dist (A × A), joint.NonNeg ∧
      (∀ pair ∈ joint.support, r pair.1 = r pair.2) ∧
      fTransform Prod.fst joint = left ∧ fTransform Prod.snd joint = right := by
  classical
  exact ⟨fibreCoupling r left right, fibreCoupling_nonNeg r hleft hright,
    fun _ member => eq_of_mem_support_fibreCoupling member,
    fTransform_fst_fibreCoupling r hleft right project,
    fTransform_snd_fibreCoupling r left hright project⟩
...
carried by the fibre relation of `r` whose marginals are the two given
laws; no non-negativity is assumed (and none is offered on the witness). -/
theorem exists_coupling_of_fTransform_eq (r : A → D) (left right : Dist A)
    (project : fTransform r left = fTransform r right) :
    ∃ joint : Dist (A × A), (∀ pair ∈ joint.support, r pair.1 = r pair.2) ∧
      fTransform Prod.fst joint = left ∧ fTransform Prod.snd joint = right := by
  classical
  -- Over the SIGNED carrier the fibrewise product above is unusable (a
  -- cancelling fibre zeroes the divisor while still carrying nonzero
  -- points), but gluing itself needs no non-negativity: transport each side
  -- onto a chosen representative of its fibre and correct once.  With
  ...
  -- `left` and `right` exactly; every support pair lies on the fibre
  -- relation because `r (rep (r a)) = r a` on inhabited fibres.
  ...
  | inr nonempty =>
      ...
      have mapDomain_sub : ∀ (g : A × A → A) (P Q : Dist (A × A)),
