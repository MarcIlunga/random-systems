...
/-- A domain filter on a sampled function-evaluator system is still a
pushforward of the original coins: filter each deterministic evaluator before
forming the law.  This is the representative-level bridge used when an
augmented observation is built over a resource-filtered system. -/
theorem PFunPDS.filterOf_functionEvaluator {Ω X Y : Type*}
    (φ : PFunPDS.DomFilter X) (p : Dist.ProbDist Ω) (F : Ω → X → Y) :
    PFunPDS.filterOf φ (PFunPDS.Prob.functionEvaluator p F).val =
      Dist.fTransform
        (fun ω => PFunDDS.filterDom φ.P φ.prefixClosed
          (PFunDDS.functionEvaluator (F ω))) p.val := by
  rw [PFunPDS.filterOf, PFunPDS.filterDom,
    PFunPDS.Prob.functionEvaluator, Dist.PMF]
  show Dist.fTransform _ (Dist.fTransform (functionEvaluatorRV F) p.val) = _
...
theorem uniformFunction_eval_uniform {X Y : Type*}
  ...
  RandomSystems.Instances.eval_nonces_uniform xs hxs
