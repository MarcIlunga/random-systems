...
theorem massYAfalse_le_massAllFalse {Shat : PFunPDS X (Y × Bool)} (hSnn : Shat.NonNeg) (i : ℕ)
    (ys : Vector Y (i + 1)) (xs : Vector X (i + 1)) :
    massYAfalse Shat i ys xs ≤ massAllFalse Shat i ys xs := by
  unfold massYAfalse massAllFalse
  exact mass_mono hSnn fun s hsp k => ⟨(hsp k).choose, (hsp k).choose_spec.2⟩
...
theorem massYAfalse_gameEnhance_eq_of_totalUpTo (T : PFunPDS X Y) (Shat : PFunPDS X (Y × Bool))
  ...
  · rw [hA, mul_zero]
    refine le_antisymm (hSnn.mass_nonneg _) ?_
    calc massYAfalse Shat i ys xs
        ≤ massAllFalse Shat i ys xs := massYAfalse_le_massAllFalse hSnn i ys xs
...
`RandomSystems.CR18.massYAfalseEq_gameEnhance_of_condEquiv` (below), which takes the **base** `S T : PFunPDS X Y` and the MBO `cond`,
...
theorem massYAfalse_gameEnhance_eq_abstract (T : PFunPDS X Y) (Shat : PFunPDS X (Y × Bool))
    ...
    (i : ℕ) (ys : Vector Y (i + 1)) (xs : Vector X (i + 1)) :
    massYAfalse (gameEnhance T Shat) i ys xs = massYAfalse Shat i ys xs := by
  exact massYAfalse_gameEnhance_eq_of_totalUpTo T Shat hSnn hCE hTprob hmono i
...
theorem winProbBehavior_congr_mass_at (W : Dist (PFunDDS.Winner X Y))
    ...
    (hw : G.weight = H.weight) {i : ℕ} (hEq : MassYAfalseEqAt G H i) :
...
theorem winProbBehavior_congr_mass (W : Dist (PFunDDS.Winner X Y)) {G H : PFunPDS X (Y × Bool)}
    (hw : G.weight = H.weight) (hEq : MassYAfalseEq G H) (i : ℕ) :
    winProbBehavior W G i = winProbBehavior W H i := by
  unfold winProbBehavior notWonProbBehavior
  rw [hw]
  congr 1
...
/-- Fixed-round mass-variant of `advantage_le_winProb_of_gameEquiv`: same PDF chain, but the game-equivalence input is only
...
theorem advantage_le_winProb_of_massYAfalseEqAt (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X (Y × Bool)) (i : ℕ)
    (hDnn : D.NonNeg) (hSnn : S.NonNeg) (hTnn : T.NonNeg)
    (hw : S.weight = T.weight) (hEq : MassYAfalseEqAt S T i)
  ...
  refine advantage_le_winProb_assemble D S T hDnn hSnn hTnn ?_ ?_ hcancel ?_ ?_
  ...
  · rw [winProb_eq_behavior _ T i hQ' hTotT, winProb_eq_behavior _ S i hQ' hTotS,
      winProbBehavior_congr_mass_at _ hw.symm hEq.symm]

/-- mass-variant **assembly helper** (`_abstract`) of `advantage_le_winProb_of_gameEquiv`: same PDF chain, with the two `≡ᵍ`
...
`advantage_le_winProb_assemble`). The `_abstract` suffix marks the non-endpoint helper role; the body is
unchanged. It is kept public because downstream files (`BlindConverter.lean`, `BlindAbsorption.lean`) must
call it to instantiate the constructed game `Ŝ`; the paper-facing endpoints are `advantage_le_winProb_of_gameEquiv`
(RelateGameDistinguishing.lean) and `RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (GameOf.lean). -/
theorem advantage_le_winProb_of_massYAfalseEq (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X (Y × Bool)) (i : ℕ)
    (hDnn : D.NonNeg) (hSnn : S.NonNeg) (hTnn : T.NonNeg)
    (hw : S.weight = T.weight) (hEq : MassYAfalseEq S T)
    (hQ : ∀ d ∈ D.support, QueriesExactly (PFunDDS.ddToDDE d) (i + 1))
    (hTotS : TotalUpTo S (i + 1)) (hTotT : TotalUpTo T (i + 1)) :
    (advantage D (PFunPDS.ignoreMBO S) (PFunPDS.ignoreMBO T) : ℝ)
      ≤ (winProb (Dist.fTransform PFunDDS.ddToDDE D) S : ℝ) := by
  ...
  · rw [winProb_ddToDDE, winProb_eq_prod_mass]
  · rw [winProb_eq_behavior _ T i hQ' hTotT, winProb_eq_behavior _ S i hQ' hTotS,
      winProbBehavior_congr_mass _ hw.symm hEq.symm i]

/-- mass-variant **assembly helper** (`_abstract`) of the adaptive corollary: `⟨S⁻|T⁻⟩(D) ≤ Γ(S)` from
`massYAfalse`-equality of the two games.

NOTE (MODELING_REVIEW #5 / fix F1.5): like `advantage_le_winProb_of_massYAfalseEq`, this takes the central content
...
`RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (GameOf.lean). -/
theorem advantage_le_maxWinProb_of_massYAfalseEq (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X (Y × Bool)) (i : ℕ)
  ...
  refine (advantage_le_winProb_of_massYAfalseEq D S T i hD.nonNeg hSnn hTnn hw hEq hQ hTotS hTotT).trans ?_
...
theorem advantage_le_maxWinProb_of_massYAfalseEqAt (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X (Y × Bool)) (i : ℕ)
    (hSnn : S.NonNeg) (hTnn : T.NonNeg)
    (hw : S.weight = T.weight) (hEq : MassYAfalseEqAt S T i)
    (hQ : ∀ d ∈ D.support, QueriesExactly (PFunDDS.ddToDDE d) (i + 1))
    (hTotS : TotalUpTo S (i + 1)) (hTotT : TotalUpTo T (i + 1)) (hD : D.isProbDist) :
    (advantage D (PFunPDS.ignoreMBO S) (PFunPDS.ignoreMBO T) : ℝ) ≤ (maxWinProb S : ℝ) := by
  refine (advantage_le_winProb_of_massYAfalseEqAt D S T i hD.nonNeg hSnn hTnn hw hEq hQ hTotS hTotT).trans ?_
  have hD' : (Dist.fTransform PFunDDS.ddToDDE D).isProbDist := by
    exact Dist.fTransform_isProbDist PFunDDS.ddToDDE hD
  exact_mod_cast winProb_le_maxWinProb (Dist.fTransform PFunDDS.ddToDDE D) S hD'
...
theorem ignoreMBO_gameEnhance (T : PFunPDS X Y) (Shat : PFunPDS X (Y × Bool))
  ...
  rw [hcongr, fTransform_fst_prod T Shat hShat.weight_eq]
    ...
    calc
      GamePerf.winProb PFunDDS.verdict D T
          = GamePerf.winProb PFunDDS.verdict D (Dist.fTransform Prod.fst (Dist.prod T Shat)) := by
            rw [fTransform_fst_prod T Shat hShat.weight_eq]
...
distinguisher. -/
theorem advantage_le_maxWinProb_of_condEquiv_of_totalUpTo
  ...
  have hEq : MassYAfalseEqAt Shat (gameEnhance T Shat) i :=
    fun ys xs => (massYAfalse_gameEnhance_eq_of_totalUpTo T Shat hShat.nonNeg hCE hT hmono i hTtot ys xs).symm
  ...
  have key := advantage_le_maxWinProb_of_massYAfalseEqAt D Shat (gameEnhance T Shat) i
    hShat.nonNeg hEnhProb.nonNeg
    (Dist.weight_eq_weight_of_isProbDist hShat hEnhProb) hEq hQ hStot hEnhTot hD
  have hVP :
...
then applies the Lemma 4.16 chain (`advantage_le_maxWinProb_of_massYAfalseEq`). Side conditions are exactly Maurer's standing
...
is `RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (in `GameOf.lean`), which takes the **base** `S T : PFunPDS X Y`,
...
theorem advantage_le_maxWinProb_of_condEquiv (D : Dist (PFunDDS.DDD X Y)) (Shat : PFunPDS X (Y × Bool))
    (T : PFunPDS X Y) (i : ℕ)
    (hShat : Shat.isProbDist) (hT : T.isProbDist) (hCE : Shat |≡ T) (hmono : MonotoneMBO Shat)
  ...
  have hEq : MassYAfalseEq Shat (gameEnhance T Shat) :=
    fun j ys xs => (massYAfalse_gameEnhance_eq_of_totalUpTo T Shat hShat.nonNeg hCE hT hmono j
      (TotalUpTo_of_totalOnNonempty hTtot (j + 1)) ys xs).symm
  have key := advantage_le_maxWinProb_of_massYAfalseEq D Shat (gameEnhance T Shat) i
    hShat.nonNeg (gameEnhance_isProbDist T Shat hT hShat).nonNeg
    (Dist.weight_eq_weight_of_isProbDist hShat (gameEnhance_isProbDist T Shat hT hShat)) hEq hQ
