...
import RandomSystems.CR18.PFunFix
...
private def queryLimitOutputFrom (q : Nat) :
    Nat → Bool →
...
private theorem queryLimitOutputFrom_prefix (q : Nat) :
...
/-- CR18 §3.4.3 / Definition 3.10: applying `[q]` to a DDS restricts the
DDS to histories of at most `q` queries and preserves the original replies. -/
def queryLimitApply (q : Nat) (S : PFunDDS.DDS X Y) : PFunDDS.DDS X Y :=
  ⟨(fun l : List X => (⟨l ∈ PFunDDS.dom S ∧ l.length ≤ q,
      fun h => PFunDDS.output S l h.1⟩ : Part Y)),
    ⟨by
      intro h
      exact PFunDDS.empty_not_mem S h.1,
    by
      intro l₁ l₂ hp hne hdom
      exact ⟨PFunDDS.prefix_closed S hp hne hdom.1,
        le_trans (List.IsPrefix.length_le hp) hdom.2⟩⟩⟩
...
private theorem queryLimit_first_query_output (q : Nat) (x : X) (h : 0 < q) :
...
private theorem queryLimit_first_query_undefined (x : X) :
...
private theorem queryLimit_reply_output (q : Nat) (x : X) (y : Y) (h : 0 < q) :
...
private def cascadeMiddle (S : DDS X Y) (l : List X) (h : l ∈ dom S) : List Y :=
...
@[simp]
private theorem cascadeMiddle_length (S : DDS X Y) (l : List X) (h : l ∈ dom S) :
    (cascadeMiddle S l h).length = l.length := by
  simp [cascadeMiddle]

private theorem cascadeMiddle_getElem (S : DDS X Y) (l : List X) (h : l ∈ dom S)
...
private theorem cascadeMiddle_congr (S : DDS X Y) {l₁ l₂ : List X} (hl : l₁ = l₂)
    (h₁ : l₁ ∈ dom S) (h₂ : l₂ ∈ dom S) :
...
private theorem cascadeMiddle_prefix (S : DDS X Y) {l₁ l₂ : List X}
...
private theorem cascadeMiddle_ne_nil (S : DDS X Y) (l : List X) (h : l ∈ dom S) :
...
private def roundsInner : List (X × Y × Z) → List (X ∪ₜ Y)
  | [] => []
  | (x, y, _) :: r => Sum.inl x :: Sum.inr y :: roundsInner r

private def roundsInputs (r : List (X × Y × Z)) : List X :=
  r.map fun p => p.1

private def roundsMiddle (r : List (X × Y × Z)) : List Y :=
  r.map fun p => p.2.1

private def roundsOutputs (r : List (X × Y × Z)) : List Z :=
  r.map fun p => p.2.2

private theorem roundsTrace_append (r₁ r₂ : List (X × Y × Z)) :
...
private theorem roundsInner_append (r₁ r₂ : List (X × Y × Z)) :
...
private theorem cascadeLeftHistory_append (l₁ l₂ : List (X ∪ₜ Y)) :
    cascadeLeftHistory (l₁ ++ l₂) =
      cascadeLeftHistory l₁ ++ cascadeLeftHistory l₂ :=
  List.filterMap_append

private theorem cascadeRightHistory_append (l₁ l₂ : List (X ∪ₜ Y)) :
    cascadeRightHistory (l₁ ++ l₂) =
      cascadeRightHistory l₁ ++ cascadeRightHistory l₂ :=
  List.filterMap_append

private theorem cascadeLeftHistory_roundsInner (r : List (X × Y × Z)) :
...
private theorem cascadeRightHistory_roundsInner (r : List (X × Y × Z)) :
...
private theorem cascadeAccess_dom_snoc (S : PFunDDS.DDS X Y) (T : PFunDDS.DDS Y Z)
...
private theorem cascadeAccess_output_inl (S : PFunDDS.DDS X Y) (T : PFunDDS.DDS Y Z)
...
private theorem cascadeAccess_output_inr (S : PFunDDS.DDS X Y) (T : PFunDDS.DDS Y Z)
...
private def stepOutput? (op : Y → Y → Y) :
...
right phase of the three-step protocol. -/
private def Valid (op : Y → Y → Y)
    (l : List ((InLabel × X) ∪ₜ
      (InLabel × Option (Sigma (fun _ : Fin 2 => Y))))) :
    Prop :=
  (stepOutput? (X := X) op l).isSome

private theorem valid_prefix (op : Y → Y → Y)
...
private def pair (S T : PFunDDS.DDS X Y) :
...
private def roundsInner :
    List (X × Y × Y) → List (Sigma (fun _ : Fin 2 => X))
  | [] => []
  | (x, _y₁, _y₂) :: r =>
      ⟨(0 : Fin 2), x⟩ :: ⟨(1 : Fin 2), x⟩ :: roundsInner r

private def roundsInputs (r : List (X × Y × Y)) : List X :=
  r.map fun p => p.1

private def roundsOutputs (op : Y → Y → Y) (r : List (X × Y × Y)) : List Y :=
  r.map fun p => op p.2.1 p.2.2

private theorem roundsTrace_append (r₁ r₂ : List (X × Y × Y)) :
    roundsTrace (r₁ ++ r₂) = roundsTrace r₁ ++ roundsTrace r₂ := by
  induction r₁ with
  | nil => rfl
  | cons p r ih =>
...
private theorem roundsInner_append (r₁ r₂ : List (X × Y × Y)) :
    roundsInner (r₁ ++ r₂) = roundsInner r₁ ++ roundsInner r₂ := by
  induction r₁ with
  | nil => rfl
  | cons p r ih =>
...
private theorem roundsInputs_append (r₁ r₂ : List (X × Y × Y)) :
    roundsInputs (r₁ ++ r₂) = roundsInputs r₁ ++ roundsInputs r₂ := by
  simp [roundsInputs, List.map_append]

private theorem roundsOutputs_append (op : Y → Y → Y) (r₁ r₂ : List (X × Y × Y)) :
    roundsOutputs op (r₁ ++ r₂) = roundsOutputs op r₁ ++ roundsOutputs op r₂ := by
  simp [roundsOutputs, List.map_append]

private theorem stepOutput?_roundsTrace_append (op : Y → Y → Y)
    (r : List (X × Y × Y))
    {x : X}
...
private theorem valid_roundsTrace_two_iff (op : Y → Y → Y)
    (r : List (X × Y × Y)) (x : X)
...
private theorem valid_roundsTrace_three_iff (op : Y → Y → Y)
...
private theorem restrict_roundsInner_zero (r : List (X × Y × Y)) :
...
private theorem restrict_roundsInner_one (r : List (X × Y × Y)) :
...
private theorem combineConverter_output_two (op : Y → Y → Y)
    (r : List (X × Y × Y)) (x : X) (y₁ : Y)
    (hc :
...
private theorem combineConverter_output_three (op : Y → Y → Y)
