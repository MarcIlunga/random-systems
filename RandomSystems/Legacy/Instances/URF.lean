...
Moved into the `Legacy` tree on 2026-07-28: this is the bounded-model URF
(over `Legacy.PDS`), consumed only by `Legacy.Applications`, `Legacy.CC`, and
`HTechnique.LegacyStatelessBridge`; the live switching development uses
`Instances.URFfunEval` and the PFun-native laws.  Declaration namespace
unchanged (`RandomSystems.Instances`). / Uniform Random DDS
...
theorem URF_eval_eq_uniform [DecidableEq Y] [Nonempty (DDS X Y 1)] [Nonempty Y] (x₀ : X) :
  ...
  have h_supp : (Finsupp.equivFunOnFinite.invFun
    (fun _ : DDS X Y 1 => (1 : ℝ) / (Fintype.card (DDS X Y 1) : ℝ))).support
    = Finset.univ := by
  ...
  rw [h_card, Nat.cast_mul, div_mul_eq_div_div,
    div_self (show (↑(Fintype.card Y ^ (Fintype.card X - 1)) : ℝ) ≠ 0 from by
...
theorem probBad_urffun_birthday_le
    ...
    probBad ((Instances.URFfun (X := X) (Y := Y) (q := q)).adaptiveTranscriptDist e) B
      ≤ (Fintype.card ι : ℝ) / (d : ℝ) := by
  have hD :
      ((Instances.URFfun (X := X) (Y := Y) (q := q)).adaptiveTranscriptDist e).NonNeg := by
    rw [urffun_aTD_eq_fTransform_uniform]
    exact Dist.uniform_nonNeg.fTransform _
  refine le_trans (probBad_iUnion_le hD B P hB) ?_
  calc ∑ p : ι,
