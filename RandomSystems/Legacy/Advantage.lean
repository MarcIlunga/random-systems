...
/-- Legacy non-negative view of the now real-valued statistical distance. -/
private noncomputable def statDistNNReal {A : Type*} [Fintype A]
    (S T : Dist A) : NNReal :=
  ⟨statDist S T, statDist_nonneg S T⟩

@[simp, norm_cast]
private theorem coe_statDistNNReal {A : Type*} [Fintype A]
    (S T : Dist A) : (statDistNNReal S T : ℝ) = statDist S T := rfl
...
def advantage [Fintype X] (S T : PDS X Y q) : NNReal :=
  Finset.sup Finset.univ
    (fun inputs => statDistNNReal (S.transcriptDist inputs) (T.transcriptDist inputs))
...
def advantageAdaptive [Fintype X] [Fintype Y] [DecidableEq Y] (S T : PDS X Y q) : NNReal :=
  Finset.sup Finset.univ
    (fun e => statDistNNReal (S.adaptiveTranscriptDist e) (T.adaptiveTranscriptDist e))
...
theorem advantage_self [Fintype X] (S : PDS X Y q) :
  ...
  · apply Finset.sup_le
    intro inputs _
    have hz : statDistNNReal (S.transcriptDist inputs)
        (S.transcriptDist inputs) = 0 := by
      apply NNReal.coe_injective
      change statDist (S.transcriptDist inputs) (S.transcriptDist inputs) = 0
      exact statDist_self _
    exact hz.le
  · exact zero_le _
...
theorem advantage_triangle [Fintype X]
  ...
  apply Finset.sup_le
  intro inputs _
  let fST := fun i => statDistNNReal (S.transcriptDist i) (T.transcriptDist i)
  let fTU := fun i => statDistNNReal (T.transcriptDist i) (U.transcriptDist i)
  ...
  have h_tu : fTU inputs ≤ Finset.sup Finset.univ fTU :=
    Finset.le_sup (Finset.mem_univ inputs)
  calc statDistNNReal (S.transcriptDist inputs) (U.transcriptDist inputs)
      ≤ statDistNNReal (S.transcriptDist inputs) (T.transcriptDist inputs) +
        statDistNNReal (T.transcriptDist inputs) (U.transcriptDist inputs) := by
        exact NNReal.coe_le_coe.mp (by simpa using (statDist_triangle
          (S.transcriptDist inputs) (T.transcriptDist inputs) (U.transcriptDist inputs)))
    _ ≤ _ := by simpa [fST, fTU] using add_le_add h_st h_tu
...
theorem advantage_le_of_pointwise [Fintype X]
    ...
    advantage S T ≤ ε := by
  simp only [advantage]
  exact Finset.sup_le (fun inputs _ =>
    NNReal.coe_le_coe.mp (by simpa using h inputs))
...
theorem advantageOn_le_of_pointwise [Fintype X]
  ...
  exact NNReal.coe_le_coe.mp (by simpa using h inputs hGood)
...
theorem advantageOn_le_advantage [Fintype X]
  ...
  exact Finset.le_sup
    (s := (Finset.univ : Finset (Fin q → X)))
      (f := fun inputs => statDistNNReal (S.transcriptDist inputs) (T.transcriptDist inputs))
...
theorem advantageAdaptive_self [Fintype X] [Fintype Y] [DecidableEq Y] (S : PDS X Y q) :
  ...
  · apply Finset.sup_le
    intro e _
    have hz : statDistNNReal (S.adaptiveTranscriptDist e)
        (S.adaptiveTranscriptDist e) = 0 := by
      apply NNReal.coe_injective
      change statDist (S.adaptiveTranscriptDist e)
          (S.adaptiveTranscriptDist e) = 0
      exact statDist_self _
    exact hz.le
  · exact zero_le _
...
theorem advantageAdaptive_triangle [Fintype X] [Fintype Y] [DecidableEq Y]
  ...
  apply Finset.sup_le
  intro e _
  let fST := fun e => statDistNNReal (S.adaptiveTranscriptDist e) (T.adaptiveTranscriptDist e)
  let fTU := fun e => statDistNNReal (T.adaptiveTranscriptDist e) (U.adaptiveTranscriptDist e)
  have h_st : fST e ≤ Finset.sup Finset.univ fST :=
    Finset.le_sup (Finset.mem_univ e)
  have h_tu : fTU e ≤ Finset.sup Finset.univ fTU :=
    Finset.le_sup (Finset.mem_univ e)
  calc statDistNNReal (S.adaptiveTranscriptDist e) (U.adaptiveTranscriptDist e)
      ≤ statDistNNReal (S.adaptiveTranscriptDist e) (T.adaptiveTranscriptDist e) +
        statDistNNReal (T.adaptiveTranscriptDist e) (U.adaptiveTranscriptDist e) := by
        exact NNReal.coe_le_coe.mp (by simpa using (statDist_triangle
          (S.adaptiveTranscriptDist e) (T.adaptiveTranscriptDist e)
          (U.adaptiveTranscriptDist e)))
    _ ≤ _ := by simpa [fST, fTU] using add_le_add h_st h_tu
...
theorem advantageAdaptive_le_of_pointwise [Fintype X] [Fintype Y] [DecidableEq Y]
  ...
  simp only [advantageAdaptive]
  exact Finset.sup_le (fun e _ => NNReal.coe_le_coe.mp (by simpa using h e))
...
theorem advantageAdaptiveOn_le_of_pointwise [Fintype X] [Fintype Y] [DecidableEq Y]
  ...
  exact NNReal.coe_le_coe.mp (by simpa using h e hGood)
...
theorem advantage_le_advantageAdaptive [Fintype X] [Fintype Y] [DecidableEq Y]
  ...
  have h_inputs :
      statDistNNReal (S.transcriptDist inputs) (T.transcriptDist inputs) =
        statDistNNReal (S.adaptiveTranscriptDist (DDE.nonadaptive inputs))
  ...
  calc statDistNNReal (S.transcriptDist inputs) (T.transcriptDist inputs)
      = statDistNNReal (S.adaptiveTranscriptDist (DDE.nonadaptive inputs))
