...
/-!
# Min-entropy, collision entropy and distance from uniform (tower level L2, `DESIGN.md` §12)

The first module of the L2 layer: the information theory mathlib does not have,
stated on the library's own `Dist A = A →₀ ℝ` and built only out of L0 (`Dist`)
and L1 (`Dist.expect`, `statDist`).

## Contents

*Renner–Coretti, "Cryptography" lecture notes (`papers/CR18_LN.pdf`) §7.2.3,
printed pp. 138–139.*  For a random variable `X` over a finite alphabet `𝒳`,
the three quantities

* `Dist.guessProb` — `p_max(X) = max_x P_X(x)`;
* `Dist.collProb` — `p_coll(X) = ∑_x P_X(x)²`, the probability that two
  independent copies of `X` collide;
* `Dist.distFromUniform` — `d(X) = ½ ∑_x |P_X(x) − 1/|𝒳||`;

their logarithms `Dist.minEntropy` (`H_∞(X) = −log₂ p_max(X)`) and
`Dist.collisionEntropy` (CR18's Rényi entropy `R(X) = −log₂ p_coll(X)`); and the
two lemmas CR18 leaves as exercises,

* **Lemma 7.6** — `Dist.one_div_card_le_collProb`, `Dist.collProb_le_guessProb`:
  `1/|𝒳| ≤ p_coll(X) ≤ p_max(X)`;
* **Lemma 7.7** — `Dist.distFromUniform_le_half_sqrt_card_mul_collProb_sub_one`:
  `d(X) ≤ ½√(|𝒳|·p_coll(X) − 1)`.

Both are Cauchy–Schwarz against the constant function, i.e. the `NonNeg`-layer
`Dist.expect_mul_sq_le_sq_mul_sq` evaluated on `Dist.uniform`.

*Maurer–Renner, "From Indifferentiability to Constructive Cryptography (and
Back)" (`papers/MauRen16.pdf`), Appendix, "Min-entropy sampling".*  The guessing
formulation of conditional min-entropy and its chain rule, promoted here off
mathlib `PMF` (where it used to live inside
`RandomSystemsCC/MauRen16Impossibility.lean`, `DESIGN.md` §12 point 4):

* `Dist.condGuessProb` — `2^{−H_∞(X|Y)} = max_f Pr[X = f(Y)]`;
* `Dist.condMinEntropy` — `H_∞(X|Y) = −log₂ max_f Pr[X = f(Y)]`;
* `Dist.minEntropy_marginal_sub_logb_card_le_condMinEntropy` — MauRen16 eq. (11),
  `H_∞(X|Y) ≥ H_∞(X) − log₂|𝒴|`, together with its exponentiated,
  logarithm-free form `Dist.condGuessProb_le_card_mul_guessProb_marginal`.

The Shannon layer (CR18 App. A.2: `H(X)`, conditional entropy, mutual
information) is deliberately *not* here; it is a separate L2 module.
...
* `Dist.guessProb X = max_x P_X(x)` and `Dist.condGuessProb X = max_f Pr[X = f(Y)]`
  — the `cond` prefix is mathlib's marker for a conditional version
  (`condExp`, `condVar`, `condDistrib`, `condCount`), so the pair spells itself,
  and it lines up with `minEntropy` / `condMinEntropy`.  `guessProb` is CR18's
  `p_max`; the two names are the same number and only one of them is defined.
* `RandomSystems.avgSuccessProb X Y g` (`RandomSystems/StatDist.lean`) — the
  *testing* quantity, renamed out of the `guessProb` family by this work.  It
  is prior-averaged over a **given** rule, which is mathlib's `avgRisk`
  register (`Mathlib/Probability/Decision/Risk/Defs.lean`) rather than
  `bayesRisk`, since `bayes` there names the *optimum*; the optimum is
  `sSup_avgSuccessProb_eq_half_add_half_statDist`.
* `RandomSystemsCC.MauRen16.econdGuessProb` — the `ℝ≥0∞`-valued twin of
  `condGuessProb` on mathlib `PMF`, kept in the impossibility proof because its
  alphabet `ℕ` is infinite and its strategies are arbitrary `PMF`s, neither of
  which the finitely supported `Dist` expresses.  The `e` prefix is mathlib's
  extended-real marker (`variance` / `evariance`).

## Hypothesis discipline (`DESIGN.md` §12)

Each statement carries the weakest of signed / `Dist.NonNeg` /
`Dist.isProbDist` at which it is true; the layer measurements behind the
convention are in `scratch/TransportProbe.lean`.  Notable calls here:
...
## L0 facts staged in this file

`marginal_apply`, `apply_le_marginal`, `marginal_isProbDist`,
`exists_pos_of_isProbDist`, `apply_le_weight` and `mass_graph_eq_sum` are facts
about `Dist` itself and belong in `RandomSystems.Dist`.  They are kept here so
that adding this module does not force a rebuild of the tree's base module
while other work is in flight; the section marks them for relocation.
-/
...
/-! ## 0. `Dist` facts this module needs — staged here, belong in `RandomSystems.Dist` -/
...
theorem bddAbove_range (X : Dist A) : BddAbove (Set.range (X : A → ℝ)) := by
  classical
...
theorem apply_le_weight {X : Dist A} (hX : X.NonNeg) (a : A) : X a ≤ X.weight := by
  classical
  by_cases ha : a ∈ X.support
...
theorem exists_pos_of_isProbDist {X : Dist A} (hX : X.isProbDist) : ∃ a, 0 < X a := by
  by_contra h
...
(CR18 Def. A.4).  `Dist.marginal` is the pushforward along `Prod.fst`, so this
is `fTransform_apply_eq_mass`.  Signed layer. -/
theorem marginal_apply (X : Dist (α × γ)) (a : α) :
    X.marginal a = X.mass fun p => p.1 = a :=
  fTransform_apply_eq_mass Prod.fst X a

/-- A joint law never charges a point more than its first marginal charges the
first coordinate.  `NonNeg` layer. -/
theorem apply_le_marginal {X : Dist (α × γ)} (hX : X.NonNeg) (a : α) (c : γ) :
    X (a, c) ≤ X.marginal a := by
  classical
  rw [marginal_apply, mass, Finsupp.sum]
  have hnn : ∀ p ∈ X.support, 0 ≤ if p.1 = a then X p else 0 := fun p _ => by
    by_cases hp : p.1 = a <;> simp [hp, hX p]
  by_cases h : (a, c) ∈ X.support
  · have hval : X (a, c) = if ((a, c) : α × γ).1 = a then X (a, c) else 0 := by simp
    rw [hval]
    exact Finset.single_le_sum (f := fun p : α × γ => if p.1 = a then X p else 0) hnn h
...
distribution. -/
theorem marginal_isProbDist {X : Dist (α × γ)} (hX : X.isProbDist) :
...
theorem mass_graph_eq_sum [Fintype γ] (X : Dist (α × γ)) (f : γ → α) :
...
`Finset.max'`: `Dist A` already carries finite support (`bddAbove_range`), so
...
def guessProb (X : Dist A) : ℝ := ⨆ a, X a

/-- Every point is charged at most `p_max`.  Signed layer. -/
theorem le_guessProb (X : Dist A) (a : A) : X a ≤ X.guessProb :=
...
theorem guessProb_le {X : Dist A} {c : ℝ} (h : ∀ a, X a ≤ c) (hc : 0 ≤ c) :
    X.guessProb ≤ c :=
  Real.iSup_le h hc

/-- `p_max` is nonnegative.  `NonNeg` layer. -/
theorem guessProb_nonneg {X : Dist A} (hX : X.NonNeg) : 0 ≤ X.guessProb :=
  Real.iSup_nonneg hX

/-- `p_max(X) ≤ 1` for a probability distribution. -/
theorem guessProb_le_one {X : Dist A} (hX : X.isProbDist) : X.guessProb ≤ 1 :=
...
needs. -/
theorem guessProb_pos {X : Dist A} (hX : X.isProbDist) : 0 < X.guessProb :=
...
def collProb (X : Dist A) : ℝ := X.expect fun a => X a

/-- `p_coll` as an ordinary sum of squares over a finite carrier. -/
theorem collProb_eq_sum [Fintype A] (X : Dist A) : X.collProb = ∑ a : A, X a ^ 2 := by
...
theorem collProb_nonneg (X : Dist A) : 0 ≤ X.collProb :=
...
theorem collProb_pos {X : Dist A} (hX : X.isProbDist) : 0 < X.collProb := by
  classical
...
the product law. -/
theorem collProb_eq_mass_prod_diag [DecidableEq A] (X : Dist A) :
...
def distFromUniform [Fintype A] (X : Dist A) : ℝ :=
  (∑ a : A, |X a - 1 / (Fintype.card A : ℝ)|) / 2

/-- `d(X) ≥ 0`.  Signed layer: a sum of absolute values. -/
theorem distFromUniform_nonneg [Fintype A] (X : Dist A) : 0 ≤ X.distFromUniform :=
...
theorem distFromUniform_eq_statDist_uniform [Fintype A] [Nonempty A] {X : Dist A}
    (hw : X.weight = 1) :
...
`Dist.expect_mul_sq_le_sq_mul_sq` evaluated on `Dist.uniform A`.  The uniform
...
`RandomSystems.DistExpect`. -/
private theorem expect_uniform_eq_sum_div [Fintype A] [Nonempty A] (f : A → ℝ) :
...
`Dist.expect_mul_sq_le_sq_mul_sq` on `Dist.uniform`, which is where the
nonnegative weights come from; `f` itself is arbitrary.

Stated at the root of `RandomSystems` rather than in the `Dist` namespace: the
...
theorem _root_.RandomSystems.sq_sum_le_card_mul_sum_sq [Fintype A] [Nonempty A] (f : A → ℝ) :
...
theorem one_div_card_le_collProb [Fintype A] [Nonempty A] {X : Dist A}
    (hw : X.weight = 1) :
...
theorem collProb_le_guessProb {X : Dist A} (hX : X.isProbDist) : X.collProb ≤ X.guessProb := by
...
def minEntropy (X : Dist A) : ℝ := -Real.logb 2 X.guessProb
...
of order 2). -/
def collisionEntropy (X : Dist A) : ℝ := -Real.logb 2 X.collProb

/-- `H_∞(X) ≥ 0`: no distribution has negative min-entropy. -/
theorem minEntropy_nonneg {X : Dist A} (hX : X.isProbDist) : 0 ≤ X.minEntropy := by
...
theorem minEntropy_le_collisionEntropy {X : Dist A} (hX : X.isProbDist) :
...
theorem collisionEntropy_le_logb_card [Fintype A] [Nonempty A] {X : Dist A}
    (hw : X.weight = 1) :
...
def condGuessProb (X : Dist (α × γ)) : ℝ := ⨆ f : γ → α, X.mass fun p => p.1 = f p.2
...
theorem le_condGuessProb {X : Dist (α × γ)} (hX : X.NonNeg) (f : γ → α) :
...
theorem condGuessProb_nonneg {X : Dist (α × γ)} (hX : X.NonNeg) : 0 ≤ X.condGuessProb :=
...
theorem condGuessProb_pos {X : Dist (α × γ)} (hX : X.isProbDist) : 0 < X.condGuessProb := by
...
theorem condGuessProb_le_card_mul_guessProb_marginal [Fintype γ] {X : Dist (α × γ)}
    (hX : X.NonNeg) :
  ...
  calc ∑ c : γ, X (f c, c)
      ≤ ∑ _c : γ, X.marginal.guessProb :=
        Finset.sum_le_sum fun c _ => (apply_le_marginal hX (f c) c).trans (le_guessProb _ _)
    _ = (Fintype.card γ : ℝ) * X.marginal.guessProb := by
        rw [Finset.sum_const, Finset.card_univ, nsmul_eq_mul]

/-- **Conditional min-entropy** `H_∞(X|Y) = −log₂ max_f Pr[X = f(Y)]`
(MauRen16 Appendix). -/
def condMinEntropy (X : Dist (α × γ)) : ℝ := -Real.logb 2 X.condGuessProb
