...
## The `fuel` parameter and CR18 Def 3.8

Every driver below is indexed by a `fuel : ℕ` allowance on consecutive inside-calls. Its paper
counterpart is Def 3.8's closing clause — "There is a finite upper bound on the number of
consecutive outputs of the form `(in, x)`" — which we keep as a *predicate* on converters
(`ProtocolFn.AnswersWithin`, quoting the clause verbatim; `StepConverter.AnswersWithin` at the
step level), never baked into the drivers. The drivers are total in `fuel` and monotone once
answered (`*_mono_le`/`*_mono`), and the paper-facing apply is the fuel-free `eventual` value,
defined exactly where some finite allowance suffices. Def 3.8's bound is thus the *witness*
that the eventual value exists; the running `fuel` variable itself has no paper name — it is
the unrolling counter of Matt's Thm 6.2 that the `eventual` quotient hides.
...
history `ys` (fed to the converter `innerStep`); `fuel` is the inside-call allowance (CR18
Def 3.8's finite-bound clause, as in `causalDrive`). Returns the outer answer `V` together with the
...
query restarts the inner-answer history at `[]`, with the same per-outer-query `fuel` allowance —
CR18 Def 3.8 bounds each in-streak, so one allowance covering every outer query exists). -/
...
/-! ### History-aware drive — the computable fast path for *stateful* converters

`driveOuter` selects its inner step by the current outer message alone (`step u`), so the fast path
it provides serves the **outer-memoryless** class only. Nothing in Matt §6 asks for that: causality
constrains a converter to read strictly-earlier inputs, and its own earlier outer messages are among
them. `driveOuterH` is the same drive with the consumed outer prefix threaded — `done` is what the
drive has already eaten, so the round opened by `u` runs `step (done ++ [u])`, the outer history
*including* the current message, which is exactly the argument `PFunConverter.ProtocolFn` hands its
converter.

The **inner** loop needs no change: `driveG` already takes an arbitrary `List Y → X ⊕ V`, so memory
enters only through *which* inner step the outer loop selects. Everything below therefore reuses
`driveG` and its lemmas verbatim, and the memoryless drive is **recovered, not forked** —
`driveOuter_eq_driveOuterH` is an equality of the two drives at the step that reads only the
history's last message, so `driveOuter` and all its consumers stay untouched instances of this one. -/

/-- **History-aware outer drive.** As `driveOuter`, but the step function receives the whole outer
history: `done` is the prefix the drive has already consumed, so the round opened by `u` runs
`step (done ++ [u])`. The nonemptiness proof is supplied by the drive itself (a round is always
opened by a message), so a stateful converter needs no junk value at the empty history. -/
def driveOuterH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) (fuel : ℕ) :
    List U → List X → List U → Part (List V × List X)
  | _,    xs, []        => Part.some ([], xs)
  | done, xs, u :: rest =>
      (driveG (step (done ++ [u]) (by simp)) S fuel xs []).bind fun r =>
        (driveOuterH step S fuel (done ++ [u]) r.2 rest).map fun rr => (r.1 :: rr.1, rr.2)

/-- Each completed outer query produces exactly one outer answer (twin of `driveOuter_length`). -/
theorem driveOuterH_length (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y)
    (fuel : ℕ) (done : List U) (xs : List X) (us : List U) {r : List V × List X}
    (h : r ∈ driveOuterH step S fuel done xs us) : r.1.length = us.length := by
  induction us generalizing done xs r with
  | nil => simp only [driveOuterH, Part.mem_some_iff] at h; subst h; simp
  | cons u rest ih =>
      simp only [driveOuterH, Part.mem_bind_iff, Part.mem_map_iff] at h
      obtain ⟨r', _hr', rr, hrr, rfl⟩ := h
      simp [ih (done ++ [u]) r'.2 hrr]

/-- The outer iteration splits over a concatenation of outer histories — the consumed prefix of the
second leg is the first leg appended to `done` (twin of `driveOuter_append`). -/
theorem driveOuterH_append (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y)
    (fuel : ℕ) (done : List U) (xs : List X) (a b : List U) :
    driveOuterH step S fuel done xs (a ++ b) =
      (driveOuterH step S fuel done xs a).bind fun ra =>
        (driveOuterH step S fuel (done ++ a) ra.2 b).map fun rb => (ra.1 ++ rb.1, rb.2) := by
  induction a generalizing done xs with
  | nil =>
      simp only [List.nil_append, driveOuterH, Part.bind_some, List.append_nil]
      refine (Part.map_id' ?_ _).symm
      intro rb; rfl
  | cons u rest ih =>
      simp only [List.cons_append, driveOuterH, ih, Part.bind_assoc, Part.bind_map,
        Part.map_bind, Part.map_map, Function.comp_def, List.cons_append, List.append_assoc,
        List.nil_append]

/-- `driveOuterH` is monotone in fuel (twin of `driveOuter_mono`; from `driveG_mono`). -/
theorem driveOuterH_mono (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y)
    {fuel : ℕ} :
    ∀ {us done : List U} {xs : List X} {r : List V × List X},
      r ∈ driveOuterH step S fuel done xs us → r ∈ driveOuterH step S (fuel + 1) done xs us := by
  intro us
  induction us with
  | nil => intro done xs r h; simpa [driveOuterH] using h
  | cons u rest ih =>
      intro done xs r h
      simp only [driveOuterH, Part.mem_bind_iff, Part.mem_map_iff] at h ⊢
      obtain ⟨r1, hr1, rr, hrr, hr⟩ := h
      exact ⟨r1, driveG_mono _ S hr1, rr, ih hrr, hr⟩

/-- `driveOuterH` is monotone up to any larger fuel. -/
theorem driveOuterH_mono_le (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y)
    {fuel fuel' : ℕ} {us done : List U} {xs : List X} {r : List V × List X} (hle : fuel ≤ fuel')
    (h : r ∈ driveOuterH step S fuel done xs us) : r ∈ driveOuterH step S fuel' done xs us := by
  obtain ⟨k, rfl⟩ := Nat.le.dest hle
  clear hle
  induction k with
  | zero => simpa using h
  | succ n ih =>
      have he : fuel + (n + 1) = (fuel + n) + 1 := by ring
      rw [he]; exact driveOuterH_mono step S ih

/-- The per-fuel applied raw function of a history-aware converter: replay from the empty consumed
prefix and the empty inner-query history, return the last outer answer (twin of `applyRawAt`). -/
def applyRawAtH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) (fuel : ℕ) :
    PFunDDS.Raw U V :=
  fun us => (driveOuterH step S fuel [] [] us).bind fun r =>
    match r.1.getLast? with
    | some v => Part.some v
    | none => Part.none

/-- Membership characterization of `applyRawAtH` (twin of `mem_applyRawAt_iff`). -/
theorem mem_applyRawAtH_iff (step : (us : List U) → us ≠ [] → List Y → X ⊕ V)
    (S : PFunDDS.Raw X Y) (fuel : ℕ) (us : List U) (v : V) :
    v ∈ applyRawAtH step S fuel us ↔
      ∃ r ∈ driveOuterH step S fuel [] [] us, r.1.getLast? = some v := by
  simp only [applyRawAtH, Part.mem_bind_iff]
  refine exists_congr fun r => and_congr_right fun _ => ?_
  cases r.1.getLast? with
  | none => simp
  | some w => simp [Part.mem_some_iff, eq_comm]

/-- **CR18 Def 3.9 for a stateful converter, per fuel.** The fuel-`fuel` applied history-aware
converter is a valid `DDS U V` — closure, no productivity needed (twin of `applyGAt`). -/
def applyGAtH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) (fuel : ℕ) :
    PFunDDS.DDS U V :=
  ⟨applyRawAtH step S fuel, by
    refine ⟨?_, ?_⟩
    · rw [PFun.mem_dom]; rintro ⟨v, hv⟩; simp [applyRawAtH, driveOuterH] at hv
    · intro l₁ l₂ hpre hne hdom
      obtain ⟨suf, rfl⟩ := hpre
      rw [PFun.mem_dom] at hdom
      obtain ⟨v, hv⟩ := hdom
      simp only [applyRawAtH, Part.mem_bind_iff] at hv
      obtain ⟨r, hr, _hvr⟩ := hv
      rw [driveOuterH_append, Part.mem_bind_iff] at hr
      obtain ⟨ra, hra, _hr2⟩ := hr
      have hlen : ra.1.length = l₁.length := driveOuterH_length step S fuel [] [] l₁ hra
      have hne1 : ra.1 ≠ [] := by
        intro hnil
...
@[simp] theorem applyGAtH_toPFun (step : (us : List U) → us ≠ [] → List Y → X ⊕ V)
    (S : PFunDDS.Raw X Y) (fuel : ℕ) : (applyGAtH step S fuel).1 = applyRawAtH step S fuel := rfl

/-- `applyRawAtH` is monotone up to any larger fuel (from `driveOuterH_mono_le`). -/
theorem applyRawAtH_mono_le (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y)
    {fuel fuel' : ℕ} {us : List U} {v : V} (hle : fuel ≤ fuel')
    (h : v ∈ applyRawAtH step S fuel us) : v ∈ applyRawAtH step S fuel' us := by
  rw [mem_applyRawAtH_iff] at h ⊢
  obtain ⟨r, hr, hlast⟩ := h
  exact ⟨r, driveOuterH_mono_le step S hle hr, hlast⟩

/-- The fuel-free applied raw function of a history-aware converter (twin of `applyRaw`). -/
noncomputable def applyRawH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V)
    (S : PFunDDS.Raw X Y) : PFunDDS.Raw U V :=
  fun us => eventual fun fuel => applyRawAtH step S fuel us

theorem mem_applyRawH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y)
    (us : List U) (v : V) :
    v ∈ applyRawH step S us ↔ ∃ fuel, v ∈ applyRawAtH step S fuel us :=
  mem_eventual (hmono := fun hle hw => applyRawAtH_mono_le step S hle hw)

/-- **CR18 Def 3.9 for a stateful converter, fuel-free.** Applying a history-aware converter to a
system function is a valid `DDS U V` — the eventual value of the finite unrolling (twin of
`applyG`). -/
noncomputable def applyGH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) :
    PFunDDS.DDS U V :=
  ⟨applyRawH step S, by
    refine ⟨?_, ?_⟩
    · rw [PFun.mem_dom]
      rintro ⟨v, hv⟩
      obtain ⟨fuel, hfuel⟩ := (mem_applyRawH step S [] v).mp hv
      refine PFunDDS.empty_not_mem (applyGAtH step S fuel) ?_
      rw [PFunDDS.dom, PFun.mem_dom]
      exact ⟨v, hfuel⟩
    · intro l₁ l₂ hpre hne hdom
      rw [PFun.mem_dom] at hdom
      obtain ⟨v, hv⟩ := hdom
      obtain ⟨fuel, hfuel⟩ := (mem_applyRawH step S l₂ v).mp hv
      have hl₂ : l₂ ∈ PFunDDS.dom (applyGAtH step S fuel) := by
        rw [PFunDDS.dom, PFun.mem_dom]; exact ⟨v, hfuel⟩
      have hl₁ := PFunDDS.prefix_closed (applyGAtH step S fuel) hpre hne hl₂
      rw [PFunDDS.dom, PFun.mem_dom] at hl₁
      obtain ⟨v', hv'⟩ := hl₁
      rw [PFun.mem_dom]
      exact ⟨v', (mem_applyRawH step S l₁ v').mpr ⟨fuel, hv'⟩⟩⟩

@[simp] theorem applyGH_toPFun (step : (us : List U) → us ≠ [] → List Y → X ⊕ V)
    (S : PFunDDS.Raw X Y) : (applyGH step S).1 = applyRawH step S := rfl

/-! #### The memoryless drive is the history-ignoring instance

Not a second mechanism: `driveOuter` **is** `driveOuterH` fed the step that reads only the outer
history's last message, as an equality of `Part`-valued drives at every `done`, and the four apply
layers inherit it verbatim. The receipt at the drive level that `ofStep_eq_ofHistoryStep` is at the
converter level. -/

/-- **`driveOuter` is the history-ignoring instance of `driveOuterH`** — for every already-consumed
prefix `done`, since a step that reads only `us.getLast` cannot see it. -/
theorem driveOuter_eq_driveOuterH (step : U → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) (fuel : ℕ)
    (done : List U) (xs : List X) (us : List U) :
    driveOuter step S fuel xs us
      = driveOuterH (fun us hne ys => step (us.getLast hne) ys) S fuel done xs us := by
  induction us generalizing done xs with
  | nil => rfl
  | cons u rest ih =>
      simp only [driveOuter, driveOuterH, List.getLast_append_singleton, ih (done ++ [u])]

/-- `applyRawAt` is the history-ignoring instance of `applyRawAtH`. -/
theorem applyRawAt_eq_applyRawAtH (step : U → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) (fuel : ℕ) :
    applyRawAt step S fuel
      = applyRawAtH (fun us hne ys => step (us.getLast hne) ys) S fuel := by
  funext us
  simp only [applyRawAt, applyRawAtH, driveOuter_eq_driveOuterH step S fuel []]

/-- `applyGAt` is the history-ignoring instance of `applyGAtH`. -/
theorem applyGAt_eq_applyGAtH (step : U → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) (fuel : ℕ) :
    applyGAt step S fuel = applyGAtH (fun us hne ys => step (us.getLast hne) ys) S fuel :=
  Subtype.ext (applyRawAt_eq_applyRawAtH step S fuel)

/-- `applyRaw` is the history-ignoring instance of `applyRawH`. -/
theorem applyRaw_eq_applyRawH (step : U → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) :
    applyRaw step S = applyRawH (fun us hne ys => step (us.getLast hne) ys) S := by
  funext us
  simp only [applyRaw, applyRawH, applyRawAt_eq_applyRawAtH step S]

/-- **`applyG` is the history-ignoring instance of `applyGH`** — the apply-level receipt that the
fast path was extended, not forked. -/
theorem applyG_eq_applyGH (step : U → List Y → X ⊕ V) (S : PFunDDS.Raw X Y) :
    applyG step S = applyGH (fun us hne ys => step (us.getLast hne) ys) S :=
  Subtype.ext (applyRaw_eq_applyRawH step S)

/-! #### Memoryless *resource*: the `Option`-valued, kernel-computable route

`driveOuterH` threads a `Part`-valued system function, and `Part`-bind does not reduce (its domain
is an `∃`, not the continuation's domain), so a `Part`-valued run does not close by `rfl` —
*measured*, on `driveG` against `functionEvaluator` at concrete data. Against a **memoryless
resource** `s : X → Y` the whole drive collapses to `Option`, and *that* run is decided by the
kernel — the sense in which this is a fast path. `causalDriveOuterH` is the twin of `causalDrive`
at the outer level, and `applyRawAtH_functionEvaluator` is the receipt that it computes the real
applied system. -/

/-- `Part.ofOption` carries `Option`-bind to `Part`-bind. -/
theorem ofOption_bind {α β : Type*} (o : Option α) (g : α → Option β) :
    Part.ofOption (o.bind g) = (Part.ofOption o).bind fun a => Part.ofOption (g a) := by
  cases o <;> simp [Part.ofOption]

/-- `Part.ofOption` carries `Option`-map to `Part`-map. -/
theorem ofOption_map {α β : Type*} (o : Option α) (f : α → β) :
    Part.ofOption (o.map f) = (Part.ofOption o).map f := by
  cases o <;> simp [Part.ofOption]

/-- **The computable history-aware outer drive**, against a memoryless resource `s : X → Y`: the
outer twin of `causalDrive`, threading the consumed outer prefix `done` and collecting the outer
answers. Total, `Option`-valued and free of `Part`, hence reducible by the kernel. -/
def causalDriveOuterH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (s : X → Y) (fuel : ℕ) :
    List U → List U → Option (List V)
  | _,    []        => some []
  | done, u :: rest =>
      (causalDrive (step (done ++ [u]) (by simp)) s fuel []).bind fun v =>
        (causalDriveOuterH step s fuel (done ++ [u]) rest).map fun vs => v :: vs

/-- The computable per-fuel applied function of a history-aware converter against a memoryless
resource: replay the outer history and read off the last outer answer. -/
def causalApplyAtH (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (s : X → Y) (fuel : ℕ)
    (us : List U) : Option V :=
  (causalDriveOuterH step s fuel [] us).bind fun vs => vs.getLast?

/-- Driving a history-aware converter through the *memoryless* system `functionEvaluator s` is the
`Option`-valued `causalDriveOuterH` on the outer-answer component (twin of
`driveG_functionEvaluator`). -/
theorem driveOuterH_functionEvaluator (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (s : X → Y)
    (fuel : ℕ) :
    ∀ (done : List U) (xs : List X) (us : List U),
      (driveOuterH step (PFunDDS.functionEvaluator s).1 fuel done xs us).map Prod.fst
        = Part.ofOption (causalDriveOuterH step s fuel done us) := by
  intro done xs us
  induction us generalizing done xs with
  | nil => simp [driveOuterH, causalDriveOuterH, Part.ofOption]
  | cons u rest ih =>
      have hbody : ∀ r : V × List X,
          ((driveOuterH step (PFunDDS.functionEvaluator s).1 fuel (done ++ [u]) r.2 rest).map
              fun rr => (r.1 :: rr.1, rr.2)).map Prod.fst
            = (Part.ofOption (causalDriveOuterH step s fuel (done ++ [u]) rest)).map
              fun vs => r.1 :: vs := by
        intro r
        rw [Part.map_map, ← ih (done ++ [u]) r.2, Part.map_map]
        rfl
      simp only [driveOuterH, causalDriveOuterH, Part.map_bind, hbody, ofOption_bind, ofOption_map]
      rw [← driveG_functionEvaluator (step (done ++ [u]) (by simp)) s fuel xs [], Part.bind_map]

/-- **The fast path computes the real applied system.** Against a memoryless resource the per-fuel
applied raw function of a history-aware converter is `Part.ofOption` of the kernel-computable
`causalApplyAtH` (the multi-query, history-aware twin of
`applyRawAt_functionEvaluator_singleton`). -/
theorem applyRawAtH_functionEvaluator (step : (us : List U) → us ≠ [] → List Y → X ⊕ V) (s : X → Y)
    (fuel : ℕ) (us : List U) :
    applyRawAtH step (PFunDDS.functionEvaluator s).1 fuel us
      = Part.ofOption (causalApplyAtH step s fuel us) := by
  unfold applyRawAtH causalApplyAtH
  rw [ofOption_bind, ← driveOuterH_functionEvaluator step s fuel [] [], Part.bind_map]
  congr 1
  funext r
  cases r.1.getLast? <;> simp [Part.ofOption]
...
theorem hinv_concat {q : ℕ} (S : Fin q → PFunDDS.DDS X Y) {i : Fin q} {xs : List (Σ _ : Fin q, X)}
