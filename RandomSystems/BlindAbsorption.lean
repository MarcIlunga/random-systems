...
This file upgrades the blind sharpening of Theorem 4.17 (`advantage_le_blindMaxWinProb_of_condEquiv_of_blind`, which
...
The closing helper `advantage_le_blindMaxWinProb_of_condEquiv` then has **exactly** the hypotheses of the
blind helper *minus* `hblind` (the public base-object endpoint that routes through it is
`RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf`, GameOf.lean; cf. MODELING_REVIEW #3 / fix F1.2).
-/
...
theorem notWonProbBehavior_absorption (D : Dist (DDD X Y)) (Shat : PFunPDS X (Y × Bool))
  ...
  have hRHS : (∑' ys : Vector Y (i + 1),
    ...
    rw [tsum_mul_left, tsum_massYAfalse_eq_massAfalse Shat hmono i xs]
  rw [hLHS, hRHS]
...
theorem winProb_absorption_of_totalUpTo (D : Dist (DDD X Y)) (Shat : PFunPDS X (Y × Bool))
  ...
  have hwL : (Dist.fTransform ddToDDE D).weight = 1 := (Dist.fTransform_isProbDist _ hD).weight_eq
  have hwGE : (gameEnhance T Shat).weight = 1 := (gameEnhance_isProbDist T Shat hT hShat).weight_eq
  have hwR : (absorbedWinnerDist D T).weight = 1 := (absorbedWinnerDist_isProbDist D T hD hT).weight_eq
  rw [hwL, hwGE, hwR, hShat.weight_eq]
...
Same per-winner absorbed bound as `advantage_le_absorbedWinnerProb_of_condEquiv`, but with the
...
theorem advantage_le_absorbedWinnerProb_of_condEquiv_of_totalUpTo
  ...
  have hEq : MassYAfalseEqAt Shat (gameEnhance T Shat) i :=
    fun ys xs => (massYAfalse_gameEnhance_eq_of_totalUpTo T Shat hShat.nonNeg hCE hT hmono i hTU ys xs).symm
  have hGEU : TotalUpTo (gameEnhance T Shat) (i + 1) :=
    gameEnhance_totalUpTo T Shat (i + 1) hTU hSU
  have key := advantage_le_winProb_of_massYAfalseEqAt D Shat (gameEnhance T Shat) i
    hD.nonNeg hShat.nonNeg (gameEnhance_isProbDist T Shat hT hShat).nonNeg
    (Dist.weight_eq_weight_of_isProbDist hShat (gameEnhance_isProbDist T Shat hT hShat)) hEq hQ hSU hGEU
  have hVP :
  ...
  have hbridge : winProb (Dist.fTransform ddToDDE D) Shat
    ...
    rw [winProb_eq_behavior _ Shat i hQ' hSU,
      ...
      winProbBehavior_congr_mass_at _
        (Dist.weight_eq_weight_of_isProbDist hShat (gameEnhance_isProbDist T Shat hT hShat)) hEq]
...
The proof opens exactly as `advantage_le_blindMaxWinProb_of_condEquiv_of_blind` (Lemma 4.16 mass form), bounding the
...
`advantage_le_blindMaxWinProb_of_condEquiv_of_blind` minus `hblind`: probability
...
`advantage_le_winProb_of_condEquiv_of_blind` shape.
...
Theorem 4.17 is `RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (GameOf.lean), which sets `Ŝ := gameOf S cond`, proves
`ignoreMBO Ŝ = S`/`MonotoneMBO Ŝ`, and routes through these absorbed helpers. The crucial faithfulness
gain over `advantage_le_blindMaxWinProb_of_condEquiv_of_blind` is that **blindness is derived** here (the absorbed
winner `D·T̃` is blind for *any* `D`, CR18_LN.txt:5694), so there is **no `hblind` hypothesis on `D`**. -/
theorem advantage_le_absorbedWinnerProb_of_condEquiv
    (D : Dist (DDD X Y)) (Shat : PFunPDS X (Y × Bool))
    (T : PFunPDS X Y) (i : ℕ)
    (hShat : Shat.isProbDist) (hT : T.isProbDist) (hCE : Shat |≡ T) (hmono : MonotoneMBO Shat)
    (hStot : CondEquiv.TotalOnNonempty Shat) (hTtot : CondEquiv.TotalOnNonempty T)
  ...
  exact advantage_le_absorbedWinnerProb_of_condEquiv_of_totalUpTo D Shat T i
...
(`advantage_le_absorbedWinnerProb_of_condEquiv`) by the single, **lossy** step of replacing `Ŝ(D·T̃)` with the
...
switching lemma) use the **per-winner** `advantage_le_absorbedWinnerProb_of_condEquiv` and bound the
...
which is the faithful improvement over `advantage_le_blindMaxWinProb_of_condEquiv_of_blind`. The `_abstract` suffix
marks its non-endpoint helper role; the base-object, in-statement-constructed public Theorem 4.17 that
calls this is `RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (GameOf.lean). -/
theorem advantage_le_blindMaxWinProb_of_condEquiv_of_totalUpTo
    (D : Dist (DDD X Y)) (Shat : PFunPDS X (Y × Bool))
    (T : PFunPDS X Y) (i : ℕ)
    (hShat : Shat.isProbDist) (hT : T.isProbDist) (hCE : Shat |≡ T) (hmono : MonotoneMBO Shat)
    (hSU : TotalUpTo Shat (i + 1)) (hTU : TotalUpTo T (i + 1))
  ...
  refine (advantage_le_absorbedWinnerProb_of_condEquiv_of_totalUpTo D Shat T i
...
open PFunDDS in
theorem advantage_le_blindMaxWinProb_of_condEquiv
    (D : Dist (DDD X Y)) (Shat : PFunPDS X (Y × Bool))
    (T : PFunPDS X Y) (i : ℕ)
    (hShat : Shat.isProbDist) (hT : T.isProbDist) (hCE : Shat |≡ T) (hmono : MonotoneMBO Shat)
    (hStot : CondEquiv.TotalOnNonempty Shat) (hTtot : CondEquiv.TotalOnNonempty T)
    (hQ : ∀ d ∈ D.support, QueriesExactly (ddToDDE d) (i + 1)) (hD : D.isProbDist) :
    (advantage D (PFunPDS.ignoreMBO Shat) T : ℝ) ≤ (Γᵇ Shat : ℝ) := by
  exact advantage_le_blindMaxWinProb_of_condEquiv_of_totalUpTo D Shat T i
    hShat hT hCE hmono
    (TotalUpTo_of_totalOnNonempty hStot (i + 1))
