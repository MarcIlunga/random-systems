...
noncomputable def blindMaxWinProb (Shat : PFunPDS X (Y × Bool)) : ℝ :=
...
/-- The defining set of `Γ(bŜ)` is bounded above (by `Ŝ`'s positive-part mass,
as for `Γ`), so `Γ(bŜ)` is a genuine least upper bound, not a junk `sSup`. -/
theorem bddAbove_blindWinProb_image (Shat : PFunPDS X (Y × Bool)) :
  ...
  refine ⟨Shat.sum fun _ gp => max gp 0, ?_⟩
  rintro x ⟨W, ⟨_, hW⟩, rfl⟩
  exact GamePerf.winProb_le_sum_posPart winsDDS W Shat hW
...
/-- `Γ(bŜ)` of a non-negative game law is non-negative: the constant-`none`
point-mass winner witnesses a non-negative member of the defining set. -/
theorem blindMaxWinProb_nonneg {Shat : PFunPDS X (Y × Bool)} (hS : Shat.NonNeg) :
    0 ≤ Γᵇ Shat := by
  refine le_trans
    (GamePerf.winProb_nonneg winsDDS
      (Dist.isProbDist_single (fun _ => (none : Option X))).nonNeg hS)
    (le_csSup (bddAbove_blindWinProb_image Shat)
      ⟨Finsupp.single (fun _ => (none : Option X)) 1, ⟨?_, Dist.isProbDist_single _⟩, rfl⟩)
  intro w hw
  have hw' : w = fun _ => (none : Option X) := by
    simpa using Finsupp.support_single_subset hw
  subst hw'
  intro l₁ l₂ _
  rfl
...
theorem blindMaxWinProb_le_maxWinProb (Shat : PFunPDS X (Y × Bool)) : Γᵇ Shat ≤ Γ Shat := by
  -- every blind winner's probability is `≤ Γ(Ŝ)` (it is a winner at all); over `ℝ` the
  -- `csSup_le` route needs the blind set nonempty, witnessed by the constant-`none`
  -- (trivially blind) point-mass winner.
  refine csSup_le ?_ ?_
  · refine ⟨winProb (Finsupp.single (fun _ => (none : Option X)) 1) Shat,
      ⟨Finsupp.single (fun _ => (none : Option X)) 1, ⟨?_, Dist.isProbDist_single _⟩, rfl⟩⟩
    intro w hw
    have hw' : w = fun _ => (none : Option X) := by
      simpa using Finsupp.support_single_subset hw
...
/-- **Generic blind-supremum reduction**: for a seed-indexed game family `G` over a seed
distribution `D`, if every blind winner's per-seed winning mass is at most `c`, then
`Γᵇ (fTransform G D) ≤ c`.  Factors the supremum/`Finsupp` bookkeeping shared by the URF collision
game (`RandomSystems.SwitchingLemma`) and the CBC-MAC game (`RandomSystems.CBCMAC`). -/
theorem blindMaxWinProb_fTransform_le {A : Type*}
    (G : A → PFunDDS.DDS X (Y × Bool)) (D : Dist A) (c : NNReal)
    (h : ∀ w : PFunDDS.Winner X Y, IsBlind w → D.mass (fun a => winsDDS w (G a)) ≤ c) :
    Γᵇ (Dist.fTransform G D) ≤ c := by
  classical
  unfold blindMaxWinProb
  refine Real.sSup_le ?_ c.coe_nonneg
  rintro _ ⟨W, ⟨hblind, hWprob⟩, rfl⟩
  unfold winProb
  simp only [winProb_fTransform_game]
  unfold GamePerf.winProb
  calc W.sum (fun w wp => D.sum fun a ap => wp * ap * if winsDDS w (G a) then 1 else 0)
      ≤ W.sum (fun w wp => wp * c) := by
        refine Finsupp.sum_le_sum fun w hw => ?_
        have hinner : (D.sum fun a ap => W w * ap * if winsDDS w (G a) then 1 else 0)
            = W w * D.mass (fun a => winsDDS w (G a)) := by
          rw [Dist.mass, Finsupp.mul_sum]
          refine Finsupp.sum_congr fun a _ => ?_
          by_cases hwin : winsDDS w (G a) <;> simp [hwin, mul_comm]
        rw [hinner]
        exact mul_le_mul_of_nonneg_left (h w (hblind w hw)) (hWprob.nonNeg w)
    _ = (W.sum fun _ wp => wp) * c := by rw [← Finsupp.sum_mul]
    _ = c := by
        have hWsum : (W.sum fun _ wp => wp) = 1 := by
          rw [← Dist.weight_eq_finsupp_sum]
          exact hWprob.weight_eq
        rw [hWsum, one_mul]
...
probability `Γ(bŜ)`. The proof is the existing Theorem-4.17 chain (`advantage_le_maxWinProb_of_massYAfalseEq`, via
...
* The `hblind`-free, base-object public theorem is `RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (GameOf.lean), which
  derives blindness via the absorbed winner (`advantage_le_blindMaxWinProb_of_condEquiv`, BlindAbsorption.lean).
* This helper retains `hblind`-on-`D` only to record the sharpening for the already-non-adaptive case. -/
theorem advantage_le_blindMaxWinProb_of_condEquiv_of_blind
  ...
  have hEq : MassYAfalseEq Shat (gameEnhance T Shat) :=
    fun j ys xs =>
      (massYAfalse_gameEnhance_eq_abstract T Shat hShat.nonNeg hCE hT hTtot hmono j ys xs).symm
  ...
  have key := advantage_le_winProb_of_massYAfalseEq D Shat (gameEnhance T Shat) i
    hD.nonNeg hShat.nonNeg (gameEnhance_isProbDist T Shat hT hShat).nonNeg
...
F1.2).** Same hypotheses as `advantage_le_blindMaxWinProb_of_condEquiv_of_blind`, but the conclusion stops at the
...
base-object, blindness-deriving per-winner endpoint is `advantage_le_absorbedWinnerProb_of_condEquiv`
(BlindAbsorption.lean), routed by the public `RandomSystems.CR18.maxAdvantage_le_blindMaxWinProb_of_condEquiv_gameOf` (GameOf.lean). Kept (proof
unchanged) as an early version. -/
theorem advantage_le_winProb_of_condEquiv_of_blind (D : Dist (PFunDDS.DDD X Y))
  ...
  have hEq : MassYAfalseEq Shat (gameEnhance T Shat) :=
    fun j ys xs =>
      (massYAfalse_gameEnhance_eq_abstract T Shat hShat.nonNeg hCE hT hTtot hmono j ys xs).symm
  have key := advantage_le_winProb_of_massYAfalseEq D Shat (gameEnhance T Shat) i
    hDnn hShat.nonNeg (gameEnhance_isProbDist T Shat hT hShat).nonNeg
    (Dist.weight_eq_weight_of_isProbDist hShat (gameEnhance_isProbDist T Shat hT hShat)) hEq hQ
    (TotalUpTo_of_totalOnNonempty hStot (i + 1))
    (gameEnhance_totalUpTo T Shat (i + 1) (TotalUpTo_of_totalOnNonempty hTtot (i + 1))
      (TotalUpTo_of_totalOnNonempty hStot (i + 1)))
