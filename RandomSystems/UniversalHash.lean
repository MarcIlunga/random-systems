...
* **`keyDist` is an arbitrary key law.**  The sources say "uniformly selected
  key", which is `Dist.uniform K`; nothing in the union bound needs it, and
...
def IsAlmostUniversalFor (H : K → M → X) (keyDist : Dist K) (Φ : X → X → Prop)
...
abbrev IsAlmostUniversal (H : K → M → X) (keyDist : Dist K) (len : M → ℕ)
    (δ : ℕ → NNReal) : Prop :=
...
abbrev IsEpsUniversal (H : K → M → X) (keyDist : Dist K) (ε : NNReal) : Prop :=
...
abbrev Is2Universal [Fintype X] (H : K → M → X) (keyDist : Dist K) : Prop :=
...
abbrev IsAlmostXorUniversal [AddGroup X] (H : K → M → X) (keyDist : Dist K)
    (len : M → ℕ) (δ : ℕ → NNReal) : Prop :=
...
theorem isEpsUniversal_iff_isAlmostUniversal_const (H : K → M → X)
    (keyDist : Dist K) (ε : NNReal) (len : M → ℕ) :
...
theorem IsAlmostXorUniversal.isAlmostUniversal [AddGroup X] {H : K → M → X}
    {keyDist : Dist K} {len : M → ℕ} {δ : ℕ → NNReal}
...
variable {H : K → M → X} {keyDist : Dist K} {Φ : X → X → Prop} {len : M → ℕ}
  {δ δ' : ℕ → NNReal}
...
theorem mass_exists_mem_le_sum (hAU : IsAlmostUniversalFor H keyDist Φ len δ)
  ...
  (Dist.mass_exists_le hkey s _).trans
...
theorem mass_exists_mem_list_le_choose_two_mul
    ...
    keyDist.mass (fun k => ∃ x ∈ l, ∃ y ∈ l, x ≠ y ∧ H k x = H k y)
      ≤ (Nat.choose q 2 : ℝ) * δ ℓ := by
  classical
  refine le_trans (Dist.mass_mono hkey
