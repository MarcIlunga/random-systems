...
noncomputable def winnerFactor (W : Dist (PFunDDS.Winner X Y)) (i : ℕ)
    (xs : Vector X (i + 1)) (ys : Vector Y (i + 1)) : ℝ :=
...
/-- If two lists agree on given prefixes but disagree on the one-longer prefixes, the appended
elements differ. -/
theorem getD_ne_of_take_eq_of_take_succ_ne {α : Type*} {l l' : List α} {j j' : ℕ} (d : α)
    (heq : l.take j = l'.take j') (hne : l.take (j + 1) ≠ l'.take (j' + 1))
    (hj : j < l.length) (hj' : j' < l'.length) :
    l.getD j d ≠ l'.getD j' d := by
  intro h
  apply hne
  rw [take_succ_get' l j hj, take_succ_get' l' j' hj', heq, List.get_eq_getElem,
    List.get_eq_getElem, ← List.getD_eq_getElem l d hj, ← List.getD_eq_getElem l' d hj', h]

/-- List helper: the last element of a length-`(m+1)` prefix is the `m`-th element. -/
theorem getLast_take_succ {α : Type*} (l : List α) (m : ℕ) (hm : m < l.length)
    {hne : l.take (m + 1) ≠ []} : (l.take (m + 1)).getLast hne = l.get ⟨m, hm⟩ := by
  rw [List.getLast_eq_getElem, List.get_eq_getElem, List.getElem_take]
  congr 1
  rw [List.length_take]; show min (m + 1) l.length - 1 = m; omega

/-- **Output on a prefix**: a `functionEvaluator` answers the prefix `l.take (m+1)` with `f` at the
last input `l.get m`. -/
theorem output_functionEvaluator_take_succ {X Y : Type*} (f : X → Y) (l : List X) (m : ℕ)
    (hm : m < l.length) (h : l.take (m + 1) ∈ dom (functionEvaluator f)) :
    output (functionEvaluator f) (l.take (m + 1)) h = f (l.get ⟨m, hm⟩) := by
  rw [output_congr (functionEvaluator f) (take_succ_get' l m hm) h
    (take_succ_get' l m hm ▸ h), functionEvaluator_output]
...
theorem mass_eq_tsum_of_unique {A ι : Type*} (P : Dist A)
  ...
  have hinner : ∀ a ∈ P.support,
    ...
    · have hz : (fun p => if Q a p then P a else 0) = (fun _ => (0 : ℝ)) := by
...
theorem winProb_eq_behavior (W : Dist (PFunDDS.Winner X Y))
  ...
  exact eq_sub_of_add_eq (winProb_add_notWonProb W G)
...
theorem winProbBehavior_congr_gameEquiv (W : Dist (PFunDDS.Winner X Y))
  ...
  have hGw : G.weight = 1 := hG.weight_eq
  have hHw : H.weight = 1 := hH.weight_eq
