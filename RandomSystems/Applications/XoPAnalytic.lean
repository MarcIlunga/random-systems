...
theorem pureVisiblePositiveError_two_summand_eq_of_eq
    ...
    ((compatibleCountNNReal y /
        ((((Fintype.card G).descFactorial 2 * (Fintype.card G).descFactorial 2 : Nat) :
            NNReal) /
          (((Fintype.card G ^ 2 : Nat) : NNReal)))) *
      (Dist.uniform (Fin 2 → G) y) -
      Dist.uniform (Fin 2 → G) y) =
    1 / (((Fintype.card G ^ 2 : Nat) : NNReal) *
      ((Fintype.card G - 1 : Nat) : NNReal)) := by
  let A : NNReal :=
    (compatibleCountNNReal y /
        ((((Fintype.card G).descFactorial 2 * (Fintype.card G).descFactorial 2 : Nat) :
            NNReal) /
          (((Fintype.card G ^ 2 : Nat) : NNReal)))) *
      (Dist.uniform (Fin 2 → G) y)
  let B : NNReal := Dist.uniform (Fin 2 → G) y
  let C : NNReal :=
    1 / (((Fintype.card G ^ 2 : Nat) : NNReal) *
      ((Fintype.card G - 1 : Nat) : NNReal))
  have hdecomp : A = C + B := by
    ...
    have hN : ((Fintype.card G : Nat) : NNReal) ≠ 0 := by
      exact_mod_cast (ne_of_gt (lt_of_lt_of_le (by norm_num) hG) : Fintype.card G ≠ 0)
    have hNm1 : (((Fintype.card G - 1 : Nat) : NNReal)) ≠ 0 := by
      exact_mod_cast (by omega : Fintype.card G - 1 ≠ 0)
    have hcast :
        ((Fintype.card G - 1 : Nat) : NNReal) = (Fintype.card G : NNReal) - 1 := by
      simp
    have hNm1' : (Fintype.card G : NNReal) - 1 ≠ 0 := by
      rw [← hcast]
      exact hNm1
    have hNdecomp :
        (Fintype.card G : NNReal) = 1 + ((Fintype.card G - 1 : Nat) : NNReal) := by
...
theorem pureVisiblePositiveError_two_summand_eq_of_ne
  ...
  have hcast1 :
    ...
    simp
  have hcast2 :
      ((Fintype.card G - 2 : Nat) : NNReal) = (Fintype.card G : NNReal) - 2 := by
    simp
  rw [← hcast1, ← hcast2]
  clear hN hNm1 hNm1' hcast1 hcast2
  have hineqNat :
...
theorem pureVisiblePositiveError_two (hG : 2 ≤ Fintype.card G) :
  ...
  have hsum :
    ...
    rw [pureVisiblePositiveError]
    apply Finset.sum_congr rfl
    intro y _
    by_cases hy : y 0 = y 1
    · rw [if_pos hy]
      exact pureVisiblePositiveError_two_summand_eq_of_eq (G := G) hG y hy
    · rw [if_neg hy]
      exact pureVisiblePositiveError_two_summand_eq_of_ne (G := G) hG y hy
  rw [hsum]
  calc
    (∑ y : Fin 2 → G, if y 0 = y 1 then c else 0)
        = (Fintype.card G : NNReal) * c := by
          calc
            (∑ y : Fin 2 → G, if y 0 = y 1 then c else 0)
            ...
            _ = (Fintype.card G : NNReal) * c := by
                  rw [Fintype.sum_prod_type]
                  simp
    _ = 1 / ((Fintype.card G : NNReal) * ((Fintype.card G - 1 : Nat) : NNReal)) := by
          subst c
          have hN : ((Fintype.card G : Nat) : NNReal) ≠ 0 := by
            exact_mod_cast
