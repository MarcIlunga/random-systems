...
def classifierWeight (X : Dist A) (κ : A → Ω) (ω : Ω) : NNReal :=
...
def classifierBlockUniform (X : Dist A) (κ : A → Ω) : Dist A :=
  Finsupp.equivFunOnFinite.invFun
    (fun a : A =>
      (Dist.fTransform κ X) (κ a) / ((classifierFiber κ (κ a)).card : NNReal))
...
def classifierFiberUniform (κ : A → Ω) (ω : Ω) : A → NNReal :=
  fun a => if κ a = ω then 1 / ((classifierFiber κ ω).card : NNReal) else 0
...
theorem classifierBlockUniform_apply
    ...
    classifierBlockUniform X κ a =
      (Dist.fTransform κ X) (κ a) / ((classifierFiber κ (κ a)).card : NNReal) := by
...
theorem classifierFiberUniform_apply_of_mem
    ...
    classifierFiberUniform κ ω a =
      1 / ((classifierFiber κ ω).card : NNReal) := by
...
theorem classifierFiberUniform_sum_of_nonempty
  ...
  have hcard_ne : (((classifierFiber κ ω).card : Nat) : NNReal) ≠ 0 := by
    rcases hω with ⟨a, ha⟩
  ...
  calc
    ...
    _ = ((classifierFiber κ ω).card : NNReal) *
        (1 / ((classifierFiber κ ω).card : NNReal)) := by
...
theorem classifierBlockUniform_eq_of_constantFibers
  ...
  have hsum :
      ∑ b ∈ classifierFiber κ (κ a), X b =
        ((classifierFiber κ (κ a)).card : NNReal) * X a := by
    calc
      ∑ b ∈ classifierFiber κ (κ a), X b =
          ∑ _b ∈ classifierFiber κ (κ a), X a := by
            ...
            intro b hb
            exact hconst b hb
      _ = ((classifierFiber κ (κ a)).card : NNReal) * X a := by
            simp [Finset.sum_const, nsmul_eq_mul]
  rw [hsum]
  ...
  have hcard_ne : (((classifierFiber κ (κ a)).card : Nat) : NNReal) ≠ 0 := by
    exact_mod_cast (Nat.ne_of_gt hcard_pos_nat)
  exact_mod_cast (mul_div_cancel_left₀ (X a) hcard_ne)
...
theorem classifierFiber_card_ne_zero_of_mem
    (κ : A → Ω) {ω : Ω} {a : A} (ha : a ∈ classifierFiber κ ω) :
    (((classifierFiber κ ω).card : Nat) : NNReal) ≠ 0 := by
  have hcard_pos_nat : 0 < (classifierFiber κ ω).card :=
    Finset.card_pos.mpr ⟨a, ha⟩
  exact_mod_cast (Nat.ne_of_gt hcard_pos_nat)
...
theorem classifierBlockUniform_fTransform
  ...
  have hNN :
    ...
    · calc
      ∑ a ∈ classifierFiber κ ω, classifierBlockUniform X κ a =
          ∑ _a ∈ classifierFiber κ ω,
            (Dist.fTransform κ X) ω / ((classifierFiber κ ω).card : NNReal) := by
            apply Finset.sum_congr rfl
            intro a ha
            rw [classifierBlockUniform_apply]
            simp only [classifierFiber, Finset.mem_filter, Finset.mem_univ, true_and] at ha
...
omit [Fintype A] in
/-- On a finset where two `NNReal`-valued functions are separately constant,
the positive-part difference commutes with summing over that finset. -/
private lemma sum_tsub_eq_tsub_sum_of_const_on_finset
    (S : Finset A) (f g : A → NNReal)
  ...
  · obtain ⟨a0, ha0⟩ := hS
    ...
    have hg0 : ∀ a ∈ S, g a = g a0 := by
      ...
      exact hg ha ha0
    calc
      ∑ a ∈ S, (f a - g a) = ∑ a ∈ S, (f a0 - g a0) := by
        apply Finset.sum_congr rfl
        intro a ha
        rw [hf0 a ha, hg0 a ha]
      _ = S.card • (f a0 - g a0) := by
        simp [Finset.sum_const]
      _ = S.card • f a0 - S.card • g a0 := by
        rw [nsmul_eq_mul, nsmul_eq_mul, nsmul_eq_mul, mul_tsub]
      _ = (∑ a ∈ S, f a) - (∑ a ∈ S, g a) := by
        congr 1
        · calc
            S.card • f a0 = ∑ a ∈ S, f a0 := by
              simp [Finset.sum_const]
            _ = ∑ a ∈ S, f a := by
              apply Finset.sum_congr rfl
              intro a ha
...
theorem statDist_eq_sum_classifierWeights_of_commonConditionals
    (X Y : Dist A) (κ : A → Ω) (wX wY : Ω → NNReal) (D : Ω → A → NNReal)
    (hX : ∀ a : A, X a = wX (κ a) * D (κ a) a)
    (hY : ∀ a : A, Y a = wY (κ a) * D (κ a) a)
    (hD : ∀ ω : Ω, ∑ a ∈ Finset.univ.filter (fun a : A => κ a = ω), D ω a = 1) :
    statDist X Y = ∑ ω : Ω, (wX ω - wY ω) := by
  simp only [statDist]
  rw [← Finset.sum_fiberwise Finset.univ κ (fun a => X a - Y a)]
  apply Finset.sum_congr rfl
  intro ω _
  calc
    ∑ a ∈ Finset.univ.filter (fun a : A => κ a = ω), (X a - Y a)
        = ∑ a ∈ Finset.univ.filter (fun a : A => κ a = ω),
            (wX ω * D ω a - wY ω * D ω a) := by
          apply Finset.sum_congr rfl
          intro a ha
    ...
    _ = ∑ a ∈ Finset.univ.filter (fun a : A => κ a = ω),
          ...
          rw [tsub_mul]
    _ = (wX ω - wY ω) *
          ∑ a ∈ Finset.univ.filter (fun a : A => κ a = ω), D ω a := by
          rw [Finset.mul_sum]
    _ = wX ω - wY ω := by
          simp [hD ω]
...
theorem exists_classifierMassCoupling_of_commonConditionals
  ...
  obtain ⟨C, hC⟩ := optimal_coupling_exists
    (Dist.fTransform κ X) (Dist.fTransform κ Y) hweights
  refine ⟨C, ?_⟩
...
theorem statDist_eq_classifierStatDist_of_constantFibers
  ...
  intro ω _
  rw [Dist.fTransform_apply_eq_sum, Dist.fTransform_apply_eq_sum]
  exact sum_tsub_eq_tsub_sum_of_const_on_finset
...
def labelFirstBlockCouplingJoint
  ...
  Finsupp.equivFunOnFinite.invFun
    (fun p : A × A =>
      if p.1 = p.2 then
        C.joint (κ p.1, κ p.2) / ((classifierFiber κ (κ p.1)).card : NNReal)
      else if κ p.1 = κ p.2 then
        0
      else
        C.joint (κ p.1, κ p.2) /
          (((classifierFiber κ (κ p.1)).card : NNReal) *
            ((classifierFiber κ (κ p.2)).card : NNReal)))
...
theorem labelFirstBlockCouplingJoint_apply
    ...
    labelFirstBlockCouplingJoint X Y κ C (a, b) =
      if a = b then
        C.joint (κ a, κ b) / ((classifierFiber κ (κ a)).card : NNReal)
      else if κ a = κ b then
        0
      else
        C.joint (κ a, κ b) /
          (((classifierFiber κ (κ a)).card : NNReal) *
            ((classifierFiber κ (κ b)).card : NNReal)) := by
...
theorem sum_labelFirstBlockCouplingJoint_same_label_right
    ...
    ∑ b ∈ classifierFiber κ (κ a),
      labelFirstBlockCouplingJoint X Y κ C (a, b) =
        C.joint (κ a, κ a) / ((classifierFiber κ (κ a)).card : NNReal) := by
...
theorem sum_labelFirstBlockCouplingJoint_same_label_left
    ...
    ∑ a ∈ classifierFiber κ (κ b),
      labelFirstBlockCouplingJoint X Y κ C (a, b) =
        C.joint (κ b, κ b) / ((classifierFiber κ (κ b)).card : NNReal) := by
...
theorem classifierMassCoupling_joint_eq_zero_of_left_empty
  ...
  exact Finset.sum_eq_zero_iff.mp hleftZero (ω, ν) (by simp)
...
theorem classifierMassCoupling_joint_eq_zero_of_right_empty
  ...
  exact Finset.sum_eq_zero_iff.mp hrightZero (ω, ν) (by simp)
      ...
      calc
        ∑ b ∈ classifierFiber κ ν,
            ...
            ∑ _b ∈ classifierFiber κ ν,
              C.joint (κ a, ν) /
                (((classifierFiber κ (κ a)).card : NNReal) *
                  ((classifierFiber κ ν).card : NNReal)) := by
              apply Finset.sum_congr rfl
              intro b hb
              rw [labelFirstBlockCouplingJoint_apply]
              have hbκ : κ b = ν := by
              ...
              have hκaν : κ a ≠ ν := by
                ...
                exact hν h.symm
              simp [hab, hκaν, hbκ]
        _ = ((classifierFiber κ ν).card : NNReal) *
              (C.joint (κ a, ν) /
                (((classifierFiber κ (κ a)).card : NNReal) *
                  ((classifierFiber κ ν).card : NNReal))) := by
              simp [Finset.sum_const, nsmul_eq_mul]
        _ = C.joint (κ a, ν) /
              ((classifierFiber κ (κ a)).card : NNReal) := by
...
theorem sum_labelFirstBlockCouplingJoint_left_fiber
    ...
    ∑ a ∈ classifierFiber κ ω,
      labelFirstBlockCouplingJoint X Y κ C (a, b) =
        C.joint (ω, κ b) / ((classifierFiber κ (κ b)).card : NNReal) := by
  by_cases hω : ω = κ b
  · subst hω
    exact sum_labelFirstBlockCouplingJoint_same_label_left X Y κ C b
  · by_cases hnonempty : (classifierFiber κ ω).Nonempty
    · have hcard_b_ne :
        (((classifierFiber κ (κ b)).card : Nat) : NNReal) ≠ 0 :=
  ...
  · have hcard_ω_ne :
    ...
    calc
      ∑ a ∈ classifierFiber κ ω, ∑ b ∈ classifierFiber κ ν,
          labelFirstBlockCouplingJoint X Y κ C (a, b) =
          ∑ _a ∈ classifierFiber κ ω,
            C.joint (ω, ν) / ((classifierFiber κ ω).card : NNReal) := by
            apply Finset.sum_congr rfl
            intro a ha
            have haκ : κ a = ω := by
              simp only [classifierFiber, Finset.mem_filter, Finset.mem_univ, true_and] at ha
...
theorem exists_fullAndOriginalClassifierMassCouplings_of_blockUniform
  ...
  obtain ⟨CLabel, hLabel⟩ := optimal_coupling_exists
    (Dist.fTransform κ X) (Dist.fTransform κ Y) hw
...
theorem visibleStatDist_eq_sum_fiberMasses_of_compatibleCount_constant
  ...
  rw [visibleStatDist_eq_classifierStatDist_of_compatibleCount_constant
    (G := G) (q := q) κ hC]
  simp [statDist, Dist.fTransform_apply_eq_sum, realVisibleDist_apply,
...
theorem visibleStatDist_eq_sum_card_countWeights_of_compatibleCount
    ...
    visibleStatDist (G := G) (q := q) =
      ∑ ω : Ω,
        ((((Finset.univ.filter (fun y : Fin q → G => κ y = ω)).card : NNReal) *
            (Cκ ω / realVisibleDenominator (G := G) q)) -
          (((Finset.univ.filter (fun y : Fin q → G => κ y = ω)).card : NNReal) *
            (1 / ((Fintype.card G ^ q : Nat) : NNReal)))) := by
  ...
  · calc
      ∑ y ∈ Finset.univ.filter (fun y : Fin q → G => κ y = ω),
          realVisibleMass (G := G) (q := q) y
          = ∑ y ∈ Finset.univ.filter (fun y : Fin q → G => κ y = ω),
              (Cκ ω / realVisibleDenominator (G := G) q) := by
            apply Finset.sum_congr rfl
            intro y hy
            simp only [Finset.mem_filter, Finset.mem_univ, true_and] at hy
            rw [realVisibleMass, hC y, hy]
      _ = (((Finset.univ.filter (fun y : Fin q → G => κ y = ω)).card : NNReal) *
            (Cκ ω / realVisibleDenominator (G := G) q)) := by
            simp [Finset.sum_const, nsmul_eq_mul]
  · calc
      ∑ y ∈ Finset.univ.filter (fun y : Fin q → G => κ y = ω),
