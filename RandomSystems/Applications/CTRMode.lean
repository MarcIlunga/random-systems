...
/-- Multi-point function fixing cardinality: the number of functions `X → X`
that agree with a target vector `ys` at `q` distinct points is `|X|^(|X| - q)`. -/
private lemma card_fiber_multipoint [Nonempty X]
    (nonces : Fin q → X) (ys : Fin q → X) (h_inj : Function.Injective nonces) :
    (Finset.univ.filter (fun f : X → X => (fun i => f (nonces i)) = ys)).card
    = Fintype.card X ^ (Fintype.card X - q) := by
  set S := Finset.univ.image nonces
  have hS_card : S.card = q := by rw [Finset.card_image_of_injective _ h_inj, Finset.card_fin]
  rw [show Fintype.card X ^ (Fintype.card X - q) = Fintype.card (↥Sᶜ → X) from by
    rw [Fintype.card_fun, Fintype.card_coe, Finset.card_compl, hS_card]]
  apply Finset.card_bij (fun f _ => fun ⟨x, hx⟩ => f x) (fun _ _ => Finset.mem_univ _)
    (fun f₁ hf₁ f₂ hf₂ h => by
      simp only [Finset.mem_filter, Finset.mem_univ, true_and] at hf₁ hf₂
      ext x; by_cases hx : x ∈ S
      · obtain ⟨i, _, rfl⟩ := Finset.mem_image.mp hx; rw [congr_fun hf₁ i, congr_fun hf₂ i]
      · exact congr_fun h ⟨x, Finset.mem_compl.mpr hx⟩)
    (fun g _ => by
      have h_ext : ∀ x ∈ S, ∃! i : Fin q, nonces i = x := by
        intro x hx; obtain ⟨i, _, rfl⟩ := Finset.mem_image.mp hx
        exact ⟨i, rfl, fun j hj => h_inj hj⟩
      refine ⟨fun x => if hx : x ∈ S then ys ((h_ext x hx).choose)
        else g ⟨x, Finset.mem_compl.mpr hx⟩, ?_, ?_⟩
      · rw [Finset.mem_filter]; exact ⟨Finset.mem_univ _, by
          ext i
          ...
          congr 1; exact h_inj hcs.1⟩
      · ext ⟨x, hx⟩
        show dite (x ∈ S) _ _ = g ⟨x, hx⟩
        rw [dif_neg (Finset.mem_compl.mp hx)])

/-- Evaluating a uniform random function at `q` distinct points produces
the uniform distribution on output vectors `Fin q → X`. -/
private lemma eval_nonces_uniform [Nonempty X]
    (nonces : Fin q → X) (h_inj : Function.Injective nonces) [Nonempty (Fin q → X)] :
    Dist.fTransform (fun f : X → X => fun i => f (nonces i))
      (Dist.uniform (X → X)) = Dist.uniform (Fin q → X) := by
  ext ys
  ...
  have h_supp : (Finsupp.equivFunOnFinite.invFun
    (fun _ : X → X => (1 : NNReal) / (Fintype.card (X → X) : NNReal))).support = Finset.univ := by
  ...
  congr 1
  · rw [card_fiber_multipoint nonces ys h_inj, Fintype.card_fun, Fintype.card_fun, Fintype.card_fin]
    simp only [Nat.cast_pow]
    have hX : (Fintype.card X : NNReal) ≠ 0 := by exact_mod_cast Fintype.card_pos (α := X).ne'
    have hq : q ≤ Fintype.card X :=
      Fintype.card_fin q ▸ Fintype.card_le_of_injective nonces h_inj
    rw [show (Fintype.card X : NNReal) ^ Fintype.card X =
      (Fintype.card X : NNReal) ^ (Fintype.card X - q) * (Fintype.card X : NNReal) ^ q from by
      rw [← pow_add, Nat.sub_add_cancel hq]]
    rw [div_mul_eq_div_div, div_self (pow_ne_zero _ hX), one_div]
...
private lemma ctr_output_uniform [Nonempty (DDS X X 1)]
  ...
  rw [eval_nonces_uniform (fun i => (inputs i).1) h_distinct]
