...
/-- Finite product lower bound used to control the falling-factorial
normalizer.  If each `f i` lies in `[0,1]`, then the product of `(1-f i)` is at
least `1 - ∑ f i`. -/
theorem prod_one_sub_ge_one_sub_sum {ι : Type*} [LinearOrder ι]
    (s : Finset ι) (f : ι → ℝ)
...
theorem descFactorial_div_pow_ge_one_sub_sum (N q : Nat)
  ...
  have hprod_eq : (∏ i ∈ Finset.range q, (1 - (i : ℝ) / (N : ℝ))) =
      ((N.descFactorial q : Nat) : ℝ) / (N : ℝ) ^ q := by
    calc
      (∏ i ∈ Finset.range q, (1 - (i : ℝ) / (N : ℝ)))
          = ∏ i ∈ Finset.range q, (((N - i : Nat) : ℝ) / (N : ℝ)) := by
            refine Finset.prod_congr rfl ?_
            intro i hi
            rw [Finset.mem_range] at hi
            have hiN : i ≤ N := le_trans hi.le hq
            have hcast : ((N - i : Nat) : ℝ) = (N : ℝ) - (i : ℝ) := by
              exact Nat.cast_sub hiN
            rw [hcast]
            field_simp [show (N : ℝ) ≠ 0 by exact_mod_cast ne_of_gt hN]
      _ = (∏ i ∈ Finset.range q, ((N - i : Nat) : ℝ)) /
          ∏ _i ∈ Finset.range q, (N : ℝ) := by
            rw [Finset.prod_div_distrib]
      _ = ((N.descFactorial q : Nat) : ℝ) / (N : ℝ) ^ q := by
