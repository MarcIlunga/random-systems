...
carries the weakest of the three distribution layers (signed `Dist`,
`Dist.NonNeg`, `Dist.isProbDist`) at which it is true.  That is a
...
* **signed → `NonNeg`, by the Jordan split.**  On `Dist A = A →₀ ℝ` mathlib's
  ...
  (`posPart_inf_negPart_eq_zero`) supplied upstream.  No `Dist.posPart` is
  introduced: re-deriving it here would leave two, which `DESIGN.md` §12
  point 4 forbids.  What is added is the `Dist`-vocabulary face — the pointwise
  formula, `NonNeg`ness of both parts, support disjointness, and the transport
  of `Dist.expect`, `Dist.weight` and `Dist.mass` across the split.
* **`NonNeg` → `isProbDist`, by normalization.**  `Dist.normalize X = |X|⁻¹ • X`
...
used at both of the levels this file works with (the scalars `ℝ`, and `Dist A`
...
The bridge that makes the lattice-ordered-group vocabulary usable on `Dist`:
the library's pointwise predicate `Dist.NonNeg` is mathlib's `0 ≤ X` for the
pointwise order on `A →₀ ℝ`. -/

/-- `Dist.NonNeg` is the `Finsupp` order relation `0 ≤ X`.  Every mathlib fact
about the ordered group `A →₀ ℝ` reaches a `NonNeg` hypothesis through this. -/
theorem nonNeg_iff_zero_le {X : Dist A} : X.NonNeg ↔ 0 ≤ X := by
  simp [NonNeg, Finsupp.le_def]

/-! ## Lift 1 — the Jordan split: signed `Dist` → two `NonNeg` distributions
...
theorem posPart_eq_self_of_nonNeg {X : Dist A} (hX : X.NonNeg) : X⁺ = X :=
  posPart_eq_self.mpr (nonNeg_iff_zero_le.mp hX)

/-- On a nonnegative distribution the split is trivial: `X⁻ = 0`. -/
theorem negPart_eq_zero_of_nonNeg {X : Dist A} (hX : X.NonNeg) : X⁻ = 0 :=
...
theorem expect_posPart_sub_expect_negPart (X : Dist A) (f : A → ℝ) :
...
theorem weight_posPart_sub_weight_negPart (X : Dist A) :
...
theorem mass_posPart_sub_mass_negPart (X : Dist A) (P : A → Prop) :
...
theorem mass_le_mass_posPart (X : Dist A) (P : A → Prop) :
...
theorem statDist_eq_weight_posPart (X Y : Dist A) :
...
`RandomSystems.Dist.eq_zero_of_weight_eq_zero` (`RandomSystems/RandomSystem.lean`)
...
theorem eq_zero_of_nonNeg_of_weight_eq_zero {X : Dist A} (hX : X.NonNeg)
...
`δ(X, Y) = 0 → (X - Y)⁺ = 0`.  The `Dist`-level form of "each one-sided excess
summand vanishes". -/
theorem posPart_eq_zero_of_statDist_eq_zero {X Y : Dist A}
...
in `RandomSystems.Coupling`. -/
theorem max_sub_eq_zero_of_statDist_eq_zero {X Y : Dist A}
    (h : statDist X Y = 0) (a : A) : max (X a - Y a) 0 = 0 := by
  simpa using
    congrArg (fun Z : Dist A => Z a) (posPart_eq_zero_of_statDist_eq_zero h)

/-! ## Lift 2 — normalization: `NonNeg` → `isProbDist` -/

/-- Weight is homogeneous: `|c · X| = c · |X|`.  Signed layer. -/
theorem weight_smul (c : ℝ) (X : Dist A) : (c • X).weight = c * X.weight := by
  unfold weight
...
theorem mass_smul (c : ℝ) (X : Dist A) (P : A → Prop) :
...
def normalize (X : Dist A) : Dist A := X.weight⁻¹ • X

/-- Pointwise value of the normalization. -/
@[simp]
theorem normalize_apply (X : Dist A) (a : A) :
...
theorem weight_normalize {X : Dist A} (hw : X.weight ≠ 0) :
    X.normalize.weight = 1 := by
  rw [normalize, weight_smul, inv_mul_cancel₀ hw]

/-- Normalization preserves nonnegativity. -/
theorem nonNeg_normalize {X : Dist A} (hX : X.NonNeg) : X.normalize.NonNeg :=
  fun a => by
    rw [normalize_apply]
...
to `X.normalize`. -/
theorem isProbDist_normalize {X : Dist A} (hX : X.NonNeg) (hw : X.weight ≠ 0) :
...
theorem mass_normalize (X : Dist A) (P : A → Prop) :
    X.normalize.mass P = X.weight⁻¹ * X.mass P := by
  rw [normalize, mass_smul]

/-- **Transport law for `mass`**: `X(P) = |X| · X̂(P)`. -/
theorem mass_eq_weight_mul_mass_normalize {X : Dist A} (hw : X.weight ≠ 0)
...
theorem _root_.ConcaveOn.le_map_expect_of_nonNeg {φ : ℝ → ℝ}
    (hφ : ConcaveOn ℝ Set.univ φ) {X : Dist A} (hX : X.NonNeg)
    (hw : X.weight ≠ 0) (f : A → ℝ) :
...
theorem _root_.ConvexOn.map_expect_le_of_nonNeg {φ : ℝ → ℝ}
    (hφ : ConvexOn ℝ Set.univ φ) {X : Dist A} (hX : X.NonNeg)
    (hw : X.weight ≠ 0) (f : A → ℝ) :
...
theorem variance_normalize_eq_expect_sq_sub_sq_expect {X : Dist A}
    (hw : X.weight ≠ 0) (f : A → ℝ) :
...
its explicit weight factor, in pure `Dist` vocabulary. -/
theorem weight_sq_mul_variance_normalize {X : Dist A} (hw : X.weight ≠ 0)
    (f : A → ℝ) :
...
theorem sq_expect_le_weight_mul_expect_sq {X : Dist A} (hX : X.NonNeg)
    (f : A → ℝ) :
...
theorem mass_ge_le_expect_posPart_div (X : Dist A) {f : A → ℝ}
