...
import RandomSystems.HTechnique.SoP.CompressionLegacy
...
noncomputable def boundedAdaptiveTranscriptAdvantage
  ...
  letI : Nonempty G := ⟨0⟩
  exact
    RandomSystems.HTechnique.boundedAdaptiveTranscriptAdvantage
      (q := q)
...
theorem boundedAdaptiveTranscriptAdvantage_bound
  ...
  exact le_trans
    (RandomSystems.HTechnique.boundedAdaptiveTranscriptAdvantage_le_adaptiveTranscriptAdvantage
      (q := q)
      (normalizedSoPRepresentative (G := G))
      (urfRepresentative (G := G)))
