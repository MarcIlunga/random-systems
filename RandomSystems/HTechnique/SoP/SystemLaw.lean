...
import RandomSystems.HTechnique.SoP.TranscriptPrefix
...
theorem transcriptSystemFactor_normalizedSoPRV_eq_realVisibleMass
  ...
  have hfact :
      (((Fintype.card G - q).factorial : Nat) : ℝ) *
          (((Fintype.card G).descFactorial q : Nat) : ℝ) =
        (((Fintype.card G).factorial : Nat) : ℝ) := by
    exact_mod_cast hfact_nat
  have hf : (((Fintype.card G - q).factorial : Nat) : ℝ) ≠ 0 := by
...
/-- The `ℝ≥0`/`ℝ` seam of the signed-carrier migration, discharged once.

This file's visible-mass hypotheses are stated where the masses live — over
`ℝ≥0`, so `1 − eps` is *truncated* subtraction — while the transcript-law
consumers now read them off the `ℝ`-valued `Dist`, where it is not.  The two
readings agree at `eps ≤ 1`; above it the `ℝ` reading is strictly *weaker*,
because its left side goes negative while both coerced masses stay
non-negative.  So the conversion is unconditional and no caller acquires a
hypothesis: `eps ≤ 1` is deliberately **not** assumed. -/
private theorem coe_one_sub_mul_le {a b eps : NNReal}
    (h : (1 - eps) * a ≤ b) :
    ((1 : ℝ) - (eps : ℝ)) * (a : ℝ) ≤ (b : ℝ) := by
  rcases le_or_gt eps 1 with heps | heps
  · have hcoe := NNReal.coe_le_coe.mpr h
    rwa [NNReal.coe_mul, NNReal.coe_sub heps, NNReal.coe_one] at hcoe
  · have hepsR : (1 : ℝ) < (eps : ℝ) := by exact_mod_cast heps
    nlinarith [a.coe_nonneg, b.coe_nonneg]
...
theorem fixedQuerySoP_oneSided_hTechnique
  ...
  intro y
  simpa using coe_one_sub_mul_le (h_lower y)
...
theorem fixedQuerySoP_transcriptLaw_lower_bound_of_visible_bound
  ...
  exact coe_one_sub_mul_le (h_lower ys)
...
attribute [aesop safe apply (rule_sets := [Cr18Total])]
  normalizedSoPProbPDS_KStepTotal
  normalizedSoPProbPDS_totalOnNonempty
