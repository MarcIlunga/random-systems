...
theorem repeatedQuerySoP_transcriptLaw_lower_bound_experiment
  ...
  -- `eps` has to be supplied, and the seam closed by `exact` rather than
  -- `apply`: this statement writes `1 - eps` with the `ℝ≥0` leaves coerced
  -- individually, so `apply`'s reducible unification cannot invert `↑?eps`
  refine transcriptLaw_ratio_of_fixedQuery_ratio
    (R := normalizedSoPRepresentative (G := G))
    (I := urfRepresentative (G := G))
    E ((q : NNReal) ^ 3 / ((Fintype.card G : NNReal)) ^ 2) (fun xs t => ?_) t
  change
    (1 - (q : NNReal) ^ 3 / ((Fintype.card G : NNReal)) ^ 2) *
...
theorem repeatedQuerySoP_experiment_bound
  ...
  exact RandomSystems.HTechnique.oneSided_hTechnique_experiment_of_fixedQuery_ratio
    (R := normalizedSoPRepresentative (G := G))
    (I := urfRepresentative (G := G))
    (E := E)
