...
-/
import RandomSystems.HTechnique.SoP.SystemLaw
...
`/h-technique` SoP application to the `RandomSystems` migration surface.
  ...
  calc (1 - epsq) *
      ...
      ≤ (1 - epsm) *
        TranscriptLawBridge.dist
          ...
          -- the signed carrier's cost: scaling by the smaller coefficient needs
          -- the ideal transcript law to be pointwise non-negative, which the
          -- `ℝ≥0` carrier used to supply structurally
          gcongr
          exact RandomSystems.CR18.PFunPDE.transcriptLawDist_transcriptLaw_nonNeg
            _ _ _ _ s
...
theorem repeatedQuerySoP_probPDE_bound
  ...
  exact RandomSystems.CR18.oneSided_hTechnique_law_experiment_of_fixedQuery_ratio
    ...
    (fun xs t => by
      -- pure coercion placement: the ratio theorem states `1 - eps` with the
      -- `ℝ≥0` leaves coerced individually, the law-experiment hypothesis with
      -- the instantiated `eps` coerced whole.  Both subtractions are already
      -- the signed ones, so this is `push_cast` and not a weakening.
      have h := repeatedQuerySoP_deterministicTranscriptLaw_lower_bound
