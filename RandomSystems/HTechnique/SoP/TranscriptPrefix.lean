...
`RandomSystems.PDS` CR18 transcript-prefix carrier
...
reduction live in `RandomSystems.HTechnique.FixedQuery`.
-/
...
/-- Simp bridge: normalize the SoP-facing prefix embedding to the generic
`FixedQuery` constructor in one step, so proofs do not unfold both layers. -/
@[simp]
theorem transcriptPrefixOfFixedInput_eq (xs ys : Fin q → G) :
    transcriptPrefixOfFixedInput xs ys = fixedInputTranscriptPrefix xs ys :=
  rfl

/-- Simp bridge: normalize the SoP-facing lift to the generic `FixedQuery`
constructor in one step. -/
@[simp]
theorem liftVisibleDist_eq [Fintype G] (xs : Fin q → G)
    (D : RandomSystems.Dist (Fin q → G)) :
    liftVisibleDist xs D = fixedInputLiftDist xs D :=
  rfl
...
theorem liftedVisible_oneSided_hTechnique [AddGroup G] [Fintype G] [Nonempty G]
  ...
  refine RandomSystems.oneSided_hTechnique_fTransform
    ...
    (transcriptPrefixOfFixedInput xs) eps ?_ ?_ ?_ h_lower
  · -- the signed carrier's cost: the ideal law is a coerced `NNReal` mass
    -- function, so non-negativity is pointwise `NNReal.coe_nonneg`
    exact fun y => (idealVisibleMass (G := G) (q := q) y).coe_nonneg
