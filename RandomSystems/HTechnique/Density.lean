...
The signed carrier makes this an unconstrained real: a bad-event mass is
non-negative only once its distribution is, so `ℝ≥0` is no longer available to a
definition taking an arbitrary `Dist`.  This alias therefore tracks
`RandomSystems.probBad`; the non-negativity it used to assert structurally is
carried by the `NonNeg` hypotheses of the theorems below, where it is used. -/
...
theorem hTechnique_ratio {A : Type*} [Fintype A]
  ...
  exact RandomSystems.hTechnique_ratio real ideal B eps h_real_nonneg
...
theorem hTechnique_expectation {A : Type*} [Fintype A]
  ...
  exact RandomSystems.hTechnique_expectation real ideal B eps h_real_nonneg
...
theorem hTechnique_eq_on_good {A : Type*} [Fintype A]
  ...
  exact RandomSystems.hTechnique_eq_on_good real ideal B h_real_nonneg
    h_ideal_nonneg h_weight h_eq
...
theorem oneSided_hTechnique {A : Type*} [Fintype A]
  ...
  exact RandomSystems.oneSided_hTechnique real ideal eps h_ideal_nonneg
...
theorem oneSided_hTechnique_fTransform {A B : Type*} [Fintype A] [Fintype B]
  ...
  exact RandomSystems.oneSided_hTechnique_fTransform real ideal f eps
    h_ideal_nonneg h_weight h_ideal_le h_lower
...
theorem oneSided_hTechnique_proper {A : Type*} [Fintype A]
    ...
    (h_lower : ∀ a, (1 - eps) * ideal a ≤ real a) :
    RandomSystems.statDist real ideal ≤ eps := by
  exact RandomSystems.oneSided_hTechnique_proper real ideal eps h_ideal_nonneg
