...
/-- The two-point direction dichotomy: not forward is inverse. -/
theorem QueryDir.eq_inv_of_ne_fwd {d : QueryDir} (h : d ≠ QueryDir.fwd) :
    d = QueryDir.inv := by
  cases d
  · exact absurd rfl h
  · rfl
...
attribute [aesop safe apply (rule_sets := [Cr18Total])]
  strongURP_KStepTotal
  strongURP_totalOnNonempty
  tweakableURP_KStepTotal
  tweakableURP_totalOnNonempty
  tweakableStrongURP_KStepTotal
  tweakableStrongURP_totalOnNonempty
