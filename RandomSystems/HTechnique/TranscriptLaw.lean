...
/-- The representative-level transcript distribution is pointwise non-negative:
each entry is an event mass under a product probability law.  On the signed
`Dist` carrier this has to be said; the `ℝ≥0` carrier supplied it structurally.
No hypothesis is needed — a `PDSRepresentative` carries its `Dist.ProbDist`, so
the witness is already in scope. -/
theorem transcriptDist_nonNeg {X : Type u} {Y : Type v} {q : Nat}
    [FiniteTranscriptSpace X Y q]
    (S : PDSRepresentative X Y)
    (E : PDERepresentative.{u, v, r} X Y) :
    (transcriptDist (q := q) S E).NonNeg :=
  RandomSystems.CR18.PFunPDE.transcriptLawDist_transcriptLaw_nonNeg
    S.prob E.prob S.rv E.rv

/-- Pointwise form of `transcriptDist_nonNeg` on the transcript *law* itself,
which is the shape the H-technique bridges below consume. -/
theorem transcriptLaw_nonneg {X : Type u} {Y : Type v} {q : Nat}
    [FiniteTranscriptSpace X Y q]
    (S : PDSRepresentative X Y)
    (E : PDERepresentative.{u, v, r} X Y)
    (t : TranscriptPrefix X Y q) :
    0 ≤ transcriptLaw S E q t := by
  simpa using transcriptDist_nonNeg (q := q) S E t
...
theorem hTechnique_ratio_experiment
  ...
  exact hTechnique_ratio
    ...
    B eps
    (PDSRepresentative.transcriptLaw_nonneg (q := k) R E)
    (PDSRepresentative.transcriptLaw_nonneg (q := k) I E)
