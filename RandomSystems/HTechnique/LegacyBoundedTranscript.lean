...
open RandomSystems.CR18 (boundedDDE boundedDDE_apply_map_some_of_length_eq
  boundedEnvironment boundedEnvironment_KQueryTotal)
...
theorem advantageAdaptive_le_boundedAdaptiveTranscriptAdvantage
  ...
  refine RandomSystems.coe_finset_sup_le
    ...
    -- the signed carrier's `statDist` is repackaged as `ℝ≥0` by its own
    -- non-negativity theorem, matching the legacy advantage's summand
    (fun e => (⟨RandomSystems.statDist (S.adaptiveTranscriptDist e)
        (T.adaptiveTranscriptDist e),
