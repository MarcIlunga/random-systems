...
`RandomSystems.HTechnique.SecurityDefs`; this file remains build-checked as
...
theorem verdictProb_single_eq_deterministicTranscriptDist_mass_of_queriesExactly
    ...
    (hS : S.KStepTotal q) :
    verdictProb (Finsupp.single d (1 : ℝ)) S.val =
...
/-- **Singleton exact-query distinguishing is bounded by its transcript
distance.**  This is the reusable point-distinguisher leaf beneath both the
ordinary and restricted-environment transcript-advantage bridges. -/
theorem advantage_single_le_deterministicTranscriptDist_statDist_of_queriesExactly
    [Fintype (PFunPDE.TranscriptPrefix X Y q)]
    (S T : PFunPDS.Prob X Y) (d : PFunDDS.DDD X Y)
    (hQ : QueriesExactly (PFunDDS.ddToDDE d) q)
    (hS : S.KStepTotal q) (hT : T.KStepTotal q) :
    advantage (Finsupp.single d (1 : ℝ)) S.val T.val ≤
      (RandomSystems.statDist
        (PFunPDS.Prob.deterministicTranscriptDist (q := q) S (PFunDDS.ddToDDE d))
        (PFunPDS.Prob.deterministicTranscriptDist (q := q) T (PFunDDS.ddToDDE d)) : ℝ) := by
  classical
  ...
  have hSverdict :
    ...
    simpa [E, accept] using
      verdictProb_single_eq_deterministicTranscriptDist_mass_of_queriesExactly
        (q := q) S d hQ hS
  have hTverdict :
      verdictProb (Finsupp.single d (1 : ℝ)) T.val =
  ...
  calc
    ...
    _ = (RandomSystems.statDist
          (PFunPDS.Prob.deterministicTranscriptDist (q := q) S (PFunDDS.ddToDDE d))
          (PFunPDS.Prob.deterministicTranscriptDist (q := q) T (PFunDDS.ddToDDE d)) : ℝ) := rfl
...
theorem advantage_single_le_adaptiveTranscriptAdvantage_of_queriesExactly
  ...
  let E : PFunPDE.QQueryEnvironment X Y q :=
    ⟨PFunDDS.ddToDDE d,
      PFunPDE.DDEKQueryTotal_of_queriesExactly (PFunDDS.ddToDDE d) hQ⟩
  exact le_trans
...
theorem verdictProb_eq_sum_single
    ...
    verdictProb D S =
      D.sum fun d dp => dp * verdictProb (Finsupp.single d (1 : ℝ)) S := by
...
theorem advantage_eq_sum_single
    ...
    advantage D S T =
      D.sum fun d dp => (dp : ℝ) * advantage (Finsupp.single d (1 : ℝ)) S T := by
  unfold advantage
...
/-- A probability mixture of point distinguishers inherits any uniform
point-distinguisher advantage bound. -/
theorem advantage_le_of_single_le
    (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X Y) (B : ℝ)
    (hD : D.isProbDist)
    (hSingle : ∀ d ∈ D.support,
      advantage (Finsupp.single d (1 : ℝ)) S T ≤ B) :
    advantage D S T ≤ B := by
  calc
    advantage D S T
        = D.sum (fun d dp =>
            (dp : ℝ) * advantage (Finsupp.single d (1 : ℝ)) S T) :=
          advantage_eq_sum_single D S T
    _ ≤ D.sum (fun _ dp => (dp : ℝ) * B) := by
          unfold Finsupp.sum
          apply Finset.sum_le_sum
          intro d hd
          exact mul_le_mul_of_nonneg_left (hSingle d hd) (hD.nonNeg d)
    _ = (D.sum fun _ dp => (dp : ℝ)) * B := by
          rw [← Finsupp.sum_mul]
    _ = B := by
...
theorem advantage_le_adaptiveTranscriptAdvantage_of_queriesExactly
    ...
    advantage D S.val T.val ≤
      PFunPDS.Prob.adaptiveTranscriptAdvantage (q := q) S T := by
  exact advantage_le_of_single_le D S.val T.val _ hD fun d hd =>
    advantage_single_le_adaptiveTranscriptAdvantage_of_queriesExactly
      (q := q) S T d (hQ d hd) hS hT

/-- **DDS-level domain-filter evaluation; candidate for upstream.**
On a history admitted by `P`, `filterDom P` has exactly the same concrete
partial-function evaluation as the base system. -/
theorem PFunDDS.filterDom_apply_eq_some_iff
    (P : List X → Prop) (hP : PrefixClosed P) (s : PFunDDS.DDS X Y)
    {xs : List X} {y : Y} (hxs : P xs) :
    (PFunDDS.filterDom P hP s).1 xs = Part.some y ↔ s.1 xs = Part.some y := by
  constructor
  · intro h
  ...
  · intro h
    ...
    have hget : (s.1 xs).get hdom = y := (Part.eq_some_iff.mp h).2
    exact Part.eq_some_iff.mpr ⟨⟨hdom, hxs⟩, hget⟩

/-- The count filter is the length-bounded instance of
`PFunDDS.filterDom_apply_eq_some_iff`. -/
theorem PFunDDS.filterQueries_apply_eq_some_iff
    (q : Nat) (s : PFunDDS.DDS X Y) {xs : List X} {y : Y}
    (hlen : xs.length ≤ q) :
    (PFunDDS.filterQueries q s).1 xs = Part.some y ↔ s.1 xs = Part.some y :=
  PFunDDS.filterDom_apply_eq_some_iff
    (fun l => l.length ≤ q) (prefixClosed_length_le q) s hlen
...
theorem PFunPDS.Prob.KStepTotal_filterQueries_of_KStepTotal
    ...
    PFunPDS.Prob.KStepTotal
      (⟨⌈q⌉ S.val, (PFunPDS.isProbDist_filterQueries q S.property)⟩ :
...
theorem PFunPDS.Prob.adaptiveTranscriptAdvantage_filterQueries_le
    ...
    PFunPDS.Prob.adaptiveTranscriptAdvantage (q := q)
        (⟨⌈q⌉ S.val, (PFunPDS.isProbDist_filterQueries q S.property)⟩ :
          PFunPDS.Prob X Y)
        (⟨⌈q⌉ T.val, (PFunPDS.isProbDist_filterQueries q T.property)⟩ :
          PFunPDS.Prob X Y) ≤
      PFunPDS.Prob.adaptiveTranscriptAdvantage (q := q) S T := by
...
theorem maxAdvantage_filterQueries_le_filtered_adaptiveTranscriptAdvantage
    ...
    (Δ(⌈q⌉ S.val, ⌈q⌉ T.val) : ℝ) ≤
      PFunPDS.Prob.adaptiveTranscriptAdvantage (q := q)
        ⟨⌈q⌉ S.val, (PFunPDS.isProbDist_filterQueries q S.property)⟩
        ⟨⌈q⌉ T.val, (PFunPDS.isProbDist_filterQueries q T.property)⟩ := by
...
theorem oneSided_hTechnique_law_experiment_of_fixedQuery_ratio
  ...
  refine RandomSystems.statDist_le_of_one_sub_mul_le
    ...
    eps (PFunPDS.Prob.transcriptDist_nonNeg (q := q) I E) h_weight
