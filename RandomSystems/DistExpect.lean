...
`Dist.expect X f = ∑_a X(a)·f(a)` — the pen-and-paper `𝔼_X[f]` of
...
the indicator/mass identity, and `Dist.variance`.
...
Like `Dist.mass` and `Dist.weight` (and unlike the old inline spelling
`∑ a, X a * f a`), `expect` sums over the finite support carried by
`Dist A = A →₀ ℝ`, so no `Fintype` hypothesis is needed; `expect_eq_sum` is
...
it is true — signed (no hypothesis), `Dist.NonNeg`, or `Dist.isProbDist` — as
...
against `Dist.toPMF` is `integral_toPMF_eq_expect` in
`RandomSystems.DistMeasure`. -/
def expect (X : Dist A) (f : A → ℝ) : ℝ :=
  X.sum fun a w => w * f a
...
theorem expect_eq_sum [Fintype A] (X : Dist A) (f : A → ℝ) :
...
theorem expect_eq_sum_of_support_subset (X : Dist A) (f : A → ℝ) {s : Finset A}
    (hs : X.support ⊆ s) :
...
hypothesis. -/
theorem expect_add_left (X Y : Dist A) (f : A → ℝ) :
...
hypothesis. -/
theorem expect_smul_left (r : ℝ) (X : Dist A) (f : A → ℝ) :
...
hypothesis. -/
theorem expect_add_right (X : Dist A) (f g : A → ℝ) :
...
theorem expect_const_mul (X : Dist A) (r : ℝ) (f : A → ℝ) :
...
hypothesis. -/
theorem expect_smul_right (X : Dist A) (r : ℝ) (f : A → ℝ) :
...
hypothesis. -/
theorem expect_sub_right (X : Dist A) (f g : A → ℝ) :
...
theorem expect_finset_sum {ι : Type*} (X : Dist A) (s : Finset ι)
    (f : ι → A → ℝ) :
...
`[DecidablePred P]` per the decidability policy of `RandomSystems.Dist`. -/
theorem expect_indicator_const (X : Dist A) (P : A → Prop) [DecidablePred P]
    (c : ℝ) :
...
is the relation of `expect` to `Dist.mass`.  Signed layer: no hypothesis. -/
theorem expect_indicator (X : Dist A) (P : A → Prop) [DecidablePred P] :
...
theorem expect_nonneg {X : Dist A} (hX : X.NonNeg) {f : A → ℝ}
    (hf : ∀ a, 0 ≤ f a) :
    0 ≤ X.expect f :=
...
mass `-1`). -/
theorem expect_mono {X : Dist A} (hX : X.NonNeg) {f g : A → ℝ}
    (h : ∀ a, f a ≤ g a) :
...
theorem expect_le_mul_weight {X : Dist A} (hX : X.NonNeg) {f : A → ℝ} {c : ℝ}
    (h : ∀ a ∈ X.support, f a ≤ c) :
...
independent of everything in this library except the `Dist` carrier. -/
theorem mass_ge_le_expect_div {X : Dist A} (hX : X.NonNeg) {f : A → ℝ}
...
nonnegative weights. -/
theorem expect_mul_sq_le_sq_mul_sq {X : Dist A} (hX : X.NonNeg) (u v : A → ℝ) :
...
`scratch/TransportProbe.lean`). -/
def variance (X : Dist A) (f : A → ℝ) : ℝ :=
  X.expect fun a => (f a - X.expect f) ^ 2

/-- Nonnegativity of the `𝔼[(f-c)²]` form for any center `c`.  `NonNeg`
layer. -/
theorem expect_sub_sq_nonneg {X : Dist A} (hX : X.NonNeg) (f : A → ℝ) (c : ℝ) :
...
theorem variance_nonneg {X : Dist A} (hX : X.NonNeg) (f : A → ℝ) :
    0 ≤ X.variance f :=
...
theorem expect_sub_sq (X : Dist A) (f : A → ℝ) (c : ℝ) :
...
theorem variance_eq_expect_sq_sub_sq_expect {X : Dist A} (hw : X.weight = 1)
    (f : A → ℝ) :
...
theorem expect_sq_sub_sq_expect_nonneg {X : Dist A} (hX : X.isProbDist)
    (f : A → ℝ) :
...
theorem _root_.ConcaveOn.le_map_expect {φ : ℝ → ℝ}
    (hφ : ConcaveOn ℝ Set.univ φ) {X : Dist A} (hX : X.isProbDist)
    (f : A → ℝ) :
...
theorem _root_.ConvexOn.map_expect_le {φ : ℝ → ℝ}
    (hφ : ConvexOn ℝ Set.univ φ) {X : Dist A} (hX : X.isProbDist)
    (f : A → ℝ) :
