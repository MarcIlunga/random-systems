...
`RandomSystems` CR18 transcript-law surface.
...
/-- **Thin wrapper over Mathlib.** Convert a query-indexed tuple to the
length-indexed vector used by CR18 transcript prefixes.  This is
`List.Vector.ofFn`; the wrapper only fixes the CR18-facing name. -/
def vectorOfFunction (x : Fin q → X) : List.Vector X q :=
  List.Vector.ofFn x

/-- **Thin wrapper over Mathlib.** Convert a CR18 length-indexed vector back to
its query-indexed function.  This is `List.Vector.get`. -/
def functionOfVector (v : List.Vector X q) : Fin q → X :=
  v.get

/-- Bridge to the Mathlib spelling.  Deliberately not `@[simp]`: the CR18
statements use the domain vocabulary `vectorOfFunction`, so that remains the
simp normal form; use this bridge to hand a goal to Mathlib's `List.Vector`
lemmas when needed. -/
theorem vectorOfFunction_eq_ofFn (x : Fin q → X) :
    vectorOfFunction x = List.Vector.ofFn x :=
  rfl

/-- Bridge to the Mathlib spelling (see `vectorOfFunction_eq_ofFn`). -/
theorem functionOfVector_eq_get (v : List.Vector X q) :
    functionOfVector v = v.get :=
  rfl

@[simp]
theorem functionOfVector_vectorOfFunction (x : Fin q → X) :
    functionOfVector (vectorOfFunction x) = x :=
  funext fun i => List.Vector.get_ofFn x i

@[simp]
theorem vectorOfFunction_functionOfVector (v : List.Vector X q) :
    vectorOfFunction (functionOfVector v) = v :=
  List.Vector.ofFn_get v

/-- **Thin wrapper over Mathlib.** The canonical equivalence between
query-indexed tuples and CR18 length-indexed vectors: this is
`(Equiv.vectorEquivFin X q).symm`. -/
def vectorFunctionEquiv (X : Type*) (q : Nat) : (Fin q → X) ≃ List.Vector X q :=
  (Equiv.vectorEquivFin X q).symm
...
/-- **Support lemma forced by formalization; candidate for upstream.** A
per-point ratio bound transfers through the exact fixed-input transcript-prefix
lift: it suffices to compare the output-vector laws *on the fixed input's output
tuple only* (the hypothesis is guarded by `t.1 = vectorOfFunction xs`).  The
off-fixed-input branch is tight: both lifted laws have zero mass there. -/
theorem fixedInputLiftDist_ratio_at
    (xs : Fin q → X) (real ideal : RandomSystems.Dist (Fin q → Y))
    (c : ℝ) (t : CR18TranscriptPrefix X Y q)
    (h : t.1 = vectorOfFunction xs →
      c * ideal (functionOfVector t.2) ≤ real (functionOfVector t.2)) :
    c * fixedInputLiftDist xs ideal t ≤ fixedInputLiftDist xs real t := by
  rcases t with ⟨xv, yv⟩
  by_cases hxv : xv = vectorOfFunction xs
  · subst xv
    have ht : (vectorOfFunction xs, yv) =
        fixedInputTranscriptPrefix xs (functionOfVector yv) := by
...
fixed-input transcript-prefix lift.  The `(1 − eps)` instance of
`fixedInputLiftDist_ratio_at`. -/
theorem fixedInputLiftDist_pointwise_lower_bound
    (xs : Fin q → X) (real ideal : RandomSystems.Dist (Fin q → Y))
    (eps : NNReal)
    (h_lower : ∀ ys : Fin q → Y, (1 - eps) * ideal ys ≤ real ys)
    (t : CR18TranscriptPrefix X Y q) :
    (1 - eps) * fixedInputLiftDist xs ideal t ≤ fixedInputLiftDist xs real t :=
  fixedInputLiftDist_ratio_at xs real ideal (1 - (eps : ℝ)) t
    (fun _ => h_lower (functionOfVector t.2))
