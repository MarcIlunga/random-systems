...
def filterQueries (q : ℕ) (S : DDS X Y) : DDS X Y :=
  ⟨fun l => ⟨(S.1 l).Dom ∧ l.length ≤ q, fun h => (S.1 l).get h.1⟩, by
    refine ⟨fun h => empty_not_mem S h.1, ?_⟩
    intro l₁ l₂ hpre hne hdom
    exact ⟨prefix_closed S hpre hne hdom.1, le_trans hpre.length_le hdom.2⟩⟩
...
(Distinct from `PFunDDS.transcript s e`, the system↔environment interaction transcript.)
