...
theorem massYAfalse_eq_zero_of_undef {G : PFunPDS X (Y × Bool)} {i : ℕ}
...
theorem massYAfalse_congr_gameEquiv {G H : PFunPDS X (Y × Bool)}
  ...
  · push_neg at hdef
    ...
    have hjH : ¬ (gamePrewinBehavior H j.1
        (ys.get j, (PFunPDS.inputPrefix xs j, PFunPDS.outputPrefix ys j))).Dom := by
      rw [← hGH']; exact hj
    rw [massYAfalse_eq_zero_of_undef ys xs j hj, massYAfalse_eq_zero_of_undef ys xs j hjH]
