...
/-- Query filtering preserves and reflects total probability mass. -/
@[simp] theorem isProbDist_filterQueries_iff (q : ℕ) (S : PFunPDS X Y) :
...
/-- Function-distribution pushforward preserves and reflects total probability mass. -/
...
theorem transcriptLawDist_weight_le_one_of_unique_jointEvent {Ω₁ : Type w} {Ω₂ : Type z}
  ...
  calc
    ...
    _ ≤ (Dist.prodProbDist pS pE).val.weight := by
          exact Dist.sum_mass_le_weight_of_pairwise_disjoint (Dist.prodProbDist pS pE).val
            (fun t ω => transcriptJointEvent S E t ω)
            (fun t u hne ω ht hu => hne (huniq t u ω ht hu))
    _ = 1 := by
          exact (Dist.prodProbDist pS pE).property
...
theorem adaptiveTranscriptAdvantage_image_bddAbove {q : Nat}
  ...
  have hstat : RandomSystems.statDist
    ...
    RandomSystems.statDist_le_weight _ _
  have hweight :
