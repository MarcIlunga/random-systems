...
open scoped Classical in
/-- Monotonicity of mass under implication. -/
theorem mass_mono {A : Type*} (X : Dist A) {P Q : A → Prop} (h : ∀ a, P a → Q a) :
...
public Lemma 4.16 is `lemma_4_16'` below, which takes only game systems + game equivalence `S ≡ᵍ T` and
**discharges** all four obligations internally. The `_abstract` suffix marks this non-endpoint helper role;
it is kept (proof unchanged) because the mass-variant `lemma_4_16'_mass_abstract` (Theorem417.lean) reuses
...
theorem lemma_4_16_assemble_abstract (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X (Y × Bool))
  ...
  have hmonoT : (Dist.prod D T).mass (fun dg => V dg ∧ A1 dg)
      ≤ (Dist.prod D T).mass A1 := mass_mono (Dist.prod D T) fun _ h => h.2
  ...
  have key : advantage D (PFunPDS.ignoreMBO S) (PFunPDS.ignoreMBO T)
    ...
    unfold advantage
    rw [hadvT, hadvS]
    push_cast [hsplitT, hsplitS]
    have := congrArg (fun z : NNReal => (z : ℝ)) hcancel
    push_cast at this
    linarith
  rw [key]
  have h0 : (0 : ℝ) ≤ ((Dist.prod D S).mass (fun dg => V dg ∧ A1 dg) : ℝ) := by positivity
...
totality). All four mechanical obligations of the assembly helper `lemma_4_16_assemble_abstract` are
...
theorem lemma_4_16' (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X (Y × Bool)) (i : ℕ)
...
`GameOf.theorem_4_17` (fix F1.1). The present lemma takes two enhanced games and `S ≡ᵍ T` directly, so it is a
Lemma-4.16 corollary, hence the name.

*Immediate*: `lemma_4_16'` bounds the advantage by `D`-as-winner's winning probability, then
`winProb_le_maxWinProb` (`Γ` is the supremum). Faithful, no extra assumptions. -/
theorem lemma_4_16_adaptive_le_maxWinProb
  ...
  refine (lemma_4_16' D S T i hS hT hST hQ hTotS hTotT).trans ?_
