...
/-- A winner-performance `winFun` **factors through the pre-winning behavior** (CR18 §4.10.1): it
depends on a game only via its pre-winning behavior (Def 4.15). This is the hypothesis of Lemma 4.15.
`winFun : PFunPDS X (Y × Bool) → NNReal` is the performance of a fixed winner across games. -/
def FactorsThroughPreWinning (winFun : PFunPDS X (Y × Bool) → NNReal) : Prop :=
  ∃ f : PFunPDS.Behavior X Y → NNReal, ∀ G, winFun G = f (gamePrewinBehavior G)

/-- **CR18 Lemma 4.15**: a winner-performance that factors through the pre-winning behavior agrees on
game-equivalent games — `G ≡ᵍ H → winFun G = winFun H`. The entire content is that `≡ᵍ` *is* equality
of pre-winning behavior (Def 4.16), so any function of it coincides; no winning-probability or MBO
machinery is needed. This is the one theorem cnl-rs binds to (the legacy-sever contract). -/
theorem gameEquiv_winFun_eq {winFun : PFunPDS X (Y × Bool) → NNReal}
  ...
  obtain ⟨f, hf⟩ := hFactors
  rw [hf G, hf H]
  exact congrArg f hEquiv
