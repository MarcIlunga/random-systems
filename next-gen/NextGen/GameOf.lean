...
import NextGen.BlindAbsorption
...
input-history predicate `A_s : X* → {0,1}`. This constructor is the CR18/NextGen way to generate that
...
absorbed blind helper (`theorem_4_17_condEquiv_absorbed_abstract`, the free-`Ŝ` proof ingredient renamed
by MODELING_REVIEW #3 / fix F1.2), which **derives** blindness via the copying converter `T̃` rather than
assuming it (no `hblind` on `D`).

(`RelateGameDistinguishing.theorem_4_17` — the misnamed Lemma-4.16 corollary — was renamed to
`lemma_4_16_adaptive_le_maxWinProb` by MODELING_REVIEW #4 / fix F1.4, freeing the canonical bare name
`theorem_4_17` for this base-object endpoint. Fix F1.1 then applied that name here, renaming the former
`theorem_4_17_base` to the bare `theorem_4_17`.) -/
theorem theorem_4_17_advantage_of_totalUpTo (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X Y)
    (cond : List (X × Y) → Bool)
  ...
  have key := theorem_4_17_condEquiv_absorbed_abstract_of_totalUpTo D (gameOf S cond) T i
    (gameOf_isProbDist S cond hS) hT hCE (monotoneMBO_gameOf S cond hcond)
    hSU hTtot hQ hD
  rwa [ignoreMBO_gameOf S cond] at key

theorem theorem_4_17_advantage (D : Dist (PFunDDS.DDD X Y)) (S T : PFunPDS X Y)
    (cond : List (X × Y) → Bool)
    (i : ℕ) (hcond : PFunDDS.MonotoneCond cond)
    (hS : S.isProbDist) (hT : T.isProbDist) (hCE : gameOf S cond |≡ T)
    ...
    (hQ : ∀ d ∈ D.support, QueriesExactly (PFunDDS.ddToDDE d) (i + 1)) (hD : D.isProbDist) :
    (advantage D S T : ℝ) ≤ (Γᵇ (gameOf S cond) : ℝ) := by
  exact theorem_4_17_advantage_of_totalUpTo D S T cond i hcond hS hT hCE
...
theorem advantage_eq_zero_of_queriesExactly_zero
  ...
  unfold advantage
  rw [verdictProb_eq_of_queriesExactly_zero D S T hQ hS hT]
  ring
...
theorem maxAdvantage_filterQueries_zero_le_of_deltaFilteredFiniteQueryNormalization
    (S T : PFunPDS X Y) {B : ℝ} (hB : 0 ≤ B)
    (hS : (⌈0⌉ S).isProbDist) (hT : (⌈0⌉ T).isProbDist)
  ...
  refine maxAdvantage_filterQueries_le_of_deltaFilteredFiniteQueryNormalization_exact
    0 S T hB hNorm ?_
  intro D _hD hQ
  rw [advantage_eq_zero_of_queriesExactly_zero D (⌈0⌉ S) (⌈0⌉ T) hQ hS hT]
  exact hB
...
step. -/
theorem theorem_4_17_of_deltaFiniteQueryNormalization
  ...
  refine maxAdvantage_le_of_deltaFiniteQueryNormalization S T
    (Γᵇ (gameOf S cond)).coe_nonneg hNorm ?_
  intro i D hD hQ
  exact theorem_4_17_advantage D S T cond i hcond hS hT hCE hStot hTtot hQ hD
...
`⌈q+1⌉S` or `⌈q+1⌉T`. -/
theorem theorem_4_17_filtered_of_deltaFilteredFiniteQueryNormalization
  ...
  refine maxAdvantage_filterQueries_le_of_deltaFilteredFiniteQueryNormalization q S T
    (Γᵇ (gameOf (⌈q + 1⌉ S) cond)).coe_nonneg hNorm ?_
  intro D hD hQ
  exact theorem_4_17_advantage_of_totalUpTo D (⌈q + 1⌉ S) (⌈q + 1⌉ T) cond q
    hcond
    ((PFunPDS.isProbDist_filterQueries_iff (q + 1) S).mpr hS)
    ((PFunPDS.isProbDist_filterQueries_iff (q + 1) T).mpr hT)
    hCE
...
theorem theorem_4_17_filtered_of_deltaFilteredFiniteQueryNormalization_all
    (q : ℕ) (S T : PFunPDS X Y) (cond : List (X × Y) → Bool)
    (hcond : PFunDDS.MonotoneCond cond) (hS : S.isProbDist) (hT : T.isProbDist)
    (hStot : CondEquiv.TotalOnNonempty S) (hTtot : CondEquiv.TotalOnNonempty T)
    (hNorm : DeltaFilteredFiniteQueryNormalization q S T) :
  ...
  | zero =>
      ...
      exact maxAdvantage_filterQueries_zero_le_of_deltaFilteredFiniteQueryNormalization S T
        (Γᵇ (gameOf (⌈0⌉ S) cond)).coe_nonneg
        ((PFunPDS.isProbDist_filterQueries_iff 0 S).mpr hS)
        ((PFunPDS.isProbDist_filterQueries_iff 0 T).mpr hT)
        hNorm
  | succ q =>
      exact theorem_4_17_filtered_of_deltaFilteredFiniteQueryNormalization q S T cond hcond
...
distinguishers of the per-`D` reduction `theorem_4_17_advantage`.
...
theorem theorem_4_17 (S T : PFunPDS X Y) (cond : List (X × Y) → Bool)
    (hcond : PFunDDS.MonotoneCond cond) (hS : S.isProbDist) (hT : T.isProbDist)
    (hStot : CondEquiv.TotalOnNonempty S) (hTtot : CondEquiv.TotalOnNonempty T)
    (hNorm : DeltaFiniteQueryNormalization S T) :
    (gameOf S cond |≡ T) → (Δ(S, T) : ℝ) ≤ (Γᵇ (gameOf S cond) : ℝ) := by
  exact theorem_4_17_of_deltaFiniteQueryNormalization S T cond hcond hS hT hStot hTtot hNorm
...
theorem eq_4_39 (S T : PFunPDS X Y) (cond : List (X × Y) → Bool)
    (hcond : PFunDDS.MonotoneCond cond)
  ...
  fun i ys xs =>
    (massYAfalse_gameEnhance_eq_abstract T (gameOf S cond) hCE hT hTtot
